'use strict';var QuixeClass=function(){var self={};function quixe_prepare(image,all_options){self.GiDispa=all_options.GiDispa;self.GiLoad=all_options.GiLoad;self.Glk=all_options.io;game_image=image;var ls=game_image.slice(0,64);var ix,val;for(ix=0;ix<ls.length;ix++){val=ls[ix].toString(16);if(val.length<2)
val="0"+val;ls[ix]=val;}
...
return null;}
function quixe_init(){if(self.vm_started){self.Glk.fatal_error("Quixe was inited twice!");return;}
...
all_options.vm.prepare(image,all_options);started=true;all_options.io.init(all_options);}
function is_inited(){return started;}
function get_library(val){switch(val){case'GlkOte':return GlkOte;case'GiDispa':return all_options.GiDispa;case'VM':return all_options.vm;case'IO':return all_options.io;}
return null;}
return{classname:'GiLoad',load_run:load_run,inited:is_inited,getlibrary:get_library,find_data_chunk:find_data_chunk,get_metadata:get_metadata,get_cover_pict:get_cover_pict,get_debug_info:get_debug_info,get_image_info:get_image_info,get_image_url:get_image_url};};var GiLoad=new GiLoadClass();try{exports.GiLoad=GiLoad;exports.GiLoadClass=GiLoadClass;}catch(ex){};