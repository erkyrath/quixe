...
function find_picture_chunk(val){return pictchunks[val];}
function find_sound_chunk(val){return sndchunks[val];}
...
else if(el.usage=="Data"&&(chunktype=="TEXT"||chunktype=="BINA")){datachunks[el.num]={data:image.slice(pos,pos+chunklen),type:chunktype};}
else if(el.usage=="Data"&&(chunktype=="FORM")){datachunks[el.num]={data:image.slice(pos-8,pos+chunklen),type:"BINA"};}
else if(el.usage=="Snd "&&(chunktype=="FORM")){var bytes=image.slice(pos-8,pos+chunklen);sndchunks[el.num]={track:parse_audio_track(bytes)};}
else if(el.usage=="Pict"&&(chunktype=="JPEG"||chunktype=="PNG ")){var img=new Image();var encoded_data=encode_base64(image.slice(pos,pos+chunklen));var mimetype=chunktype.toLowerCase();while(mimetype[mimetype.length-1]==" "){mimetype=mimetype.slice(0,-1);}
img.onload=(function(img,num){return function(){pictchunks[num].width=img.width;pictchunks[num].height=img.height;};})(img,el.num);img.src="data:image/"+chunktype.toLowerCase()+";base64,"+encoded_data;pictchunks[el.num]={image:img,type:chunktype};}}
return result;}
...
function parse_audio_track(bytes){if(!audio_context)
return null;var aiff=new AIFFDecoder().decode(encode_raw_text(bytes));var buffer=audio_context.createBuffer(aiff.channels.length,aiff.length,aiff.sampleRate);for(var channel=0;channel<aiff.channels.length;channel++){var channel_data=buffer.getChannelData(channel);channel_data.set(aiff.channels[channel]);}
return buffer;}
function decode_raw_text(str){var arr=Array(str.length);var ix;for(ix=0;ix<str.length;ix++){arr[ix]=str.charCodeAt(ix)&0xFF;}
return arr;}
function encode_raw_text(bytes){var blocks=[];for(var i=0,l=bytes.length;i<l;i+=256){blocks.push(String.fromCharCode.apply(String,bytes.slice(i,i+256)));}
return blocks.join('');}
var decode_base64,encode_base64;if(window.atob){decode_base64=function(base64data){var data=atob(base64data);var image=Array(data.length);var ix;for(ix=0;ix<data.length;ix++)
image[ix]=data.charCodeAt(ix);return image;}
encode_base64=function(image){var blocks=[];for(var i=0,l=image.length;i<l;i+=256){blocks.push(String.fromCharCode.apply(String,image.slice(i,i+256)));}
return btoa(blocks.join(''));}}
else{var b64encoder="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";var b64decoder=(function(){var out=[];var ix;for(ix=0;ix<b64encoder.length;ix++)
...
if(e4==64)
out.pop();if(e3==64)
out.pop();return out;};encode_base64=function(bytes){var out=[];var c1,c2,c3,e1,e2,e3,e4;for(var i=0,l=bytes.length;i<l;i+=3){c1=bytes[i];c2=bytes[i+1]||0;c3=bytes[i+2]||0;e1=b64encoder.charAt(c1>>2);e2=b64encoder.charAt(((c1&0x03)<<4)+(c2>>4));e3=b64encoder.charAt(((c2&0x0f)<<2)+(c3>>6));e4=b64encoder.charAt(c3&0x3f);out.push(e1,e2,e3,e4);}
return out.join('');};}
...
return{load_run:load_run,find_data_chunk:find_data_chunk,find_picture_chunk:find_picture_chunk,find_sound_chunk:find_sound_chunk,};}();