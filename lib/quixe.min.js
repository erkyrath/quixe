...
function do_gestalt(val,val2){var ix;switch(val){case 0:return 0x00030102;case 1:return 0x00020000;case 2:return 1;case 3:return 1;case 4:switch(val2){case 0:return 1;case 1:return 1;case 2:return 1;default:return 0;}
...
cmem=compress_bytes(cmem);cmem.splice(0,0,0,0,0,0);ByteWrite4(cmem,0,endmem);chunks.push({key:"CMem",chunk:cmem});var stkschunk=[];chunks.push({key:"Stks",chunk:stkschunk});for(var i=0;i<stack.length;i++){push_serialized_stackframe(stack[i],stkschunk);}
...
jQuery.extend(all_options,optobj);gameurl=null;if(all_options.use_query_story){var qparams=get_query_params();gameurl=qparams['story'];}
...
var old_js_url=gameurl.match(/[.]js$/i);GlkOte.log('### is_relative='+is_relative+', same_origin='+same_origin+', binary_supported='+binary_supported+', crossorigin_supported='+crossorigin_supported);if(old_js_url&&same_origin){GlkOte.log('### trying old-fashioned load...');window.processBase64Zcode=function(val){start_game(decode_base64(val));};jQuery.ajax(gameurl,{'type':'GET',dataType:'script',cache:true,error:function(jqxhr,textstatus,errorthrown){all_options.io.fatal_error("The story could not be loaded. ("+gameurl+"): Error "+textstatus+": "+errorthrown);}});return;}
if(old_js_url){GlkOte.log('### trying script load...');window.processBase64Zcode=function(val){start_game(decode_base64(val));};var headls=$('head');if(!headls.length){all_options.io.fatal_error("This page has no <head> element!");return;}
var script=$('<script>',{src:gameurl,'type':"text/javascript"});headls.get(0).appendChild(script.get(0));return;}
if(binary_supported&&same_origin){GlkOte.log('### trying binary load...');jQuery.ajax(gameurl,{'type':'GET',beforeSend:function(jqxhr,settings){jqxhr.overrideMimeType('text/plain; charset=x-user-defined');},success:function(response,textstatus,errorthrown){start_game(decode_raw_text(response));},error:function(jqxhr,textstatus,errorthrown){all_options.io.fatal_error("The story could not be loaded. ("+gameurl+"): Error "+textstatus+": "+errorthrown);}});return;}
...
if(crossorigin_supported){GlkOte.log('### trying proxy load... ('+all_options.proxy_url+')');jQuery.ajax(all_options.proxy_url,{'type':'GET',data:{encode:'base64',url:absgameurl},error:function(jqxhr,textstatus,errorthrown){all_options.io.fatal_error("The story could not be loaded. ("+gameurl+"): Error "+textstatus+": "+errorthrown);},success:function(response,textstatus,errorthrown){start_game(decode_base64(response));}});return;}
...
return map;}
function absolutize(url){var div=document.createElement('div');div.innerHTML='<a></a>';div.firstChild.href=url;div.innerHTML=div.innerHTML;return div.firstChild.href;}
function get_metadata(val){return metadata[val];}
function get_image_info(val){if(window.GiLoadImages){var img=window.GiLoadImages[val];if(img)
return img;}
...
return undefined;}
function get_image_url(val){var chunk=blorbchunks['Pict:'+val];if(chunk){if(chunk.dataurl)
...
if(window.GiLoadImages){var img=window.GiLoadImages[val];if(img&&img.url)
return img.url;}
return undefined;}
...
out.pop();if(e3==64)
out.pop();return out;}}
if(window.btoa){encode_base64=function(arr){return window.btoa(String.fromCharCode.apply(this,arr));}}
else{encode_base64=function(arr){var coder="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";var res=[];for(var ix=0;ix<arr.length;ix+=3){var byte0=arr[ix];var byte1=arr[ix+1];var byte2=arr[ix+2];var count=3;if(byte2===undefined){byte2=0;count=2;}
if(byte1===undefined){byte1=0;count=1;}
res.push(coder.charAt((byte0>>2)&0x3F));res.push(coder.charAt(((byte0<<4)&0x30)|((byte1>>4)&0x0F)));if(count==1){res.push('=');res.push('=');}
else{res.push(coder.charAt(((byte1<<2)&0x3C)|((byte2>>6)&0x03)));if(count==2){res.push('=');}
else{res.push(coder.charAt((byte2)&0x3F));}}}
return res.join('');}}
