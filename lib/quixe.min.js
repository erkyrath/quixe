'use strict';var QuixeClass=function(){var self={};function quixe_init(image,all_options){self.GiDispa=all_options.GiDispa;self.GiLoad=all_options.GiLoad;self.Glk=all_options.io;game_image=image;var ls=game_image.slice(0,64);var ix,val;for(ix=0;ix<ls.length;ix++){val=ls[ix].toString(16);if(val.length<2)
val="0"+val;ls[ix]=val;}
...
else{srand_set_seed(val);self.random_func=xo_random_func;}}
...
return srand_get_random()%arg;else if(arg<0)
return-(srand_get_random()%-arg);else
return srand_get_random();}
var xo_table=undefined;function srand_set_seed(seed){var ix,s;if(xo_table===undefined)
xo_table=Array(4);seed=BigInt(seed>>>0);for(ix=0;ix<4;ix++){seed=(seed+0x9E3779B9n)&0xFFFFFFFFn;s=seed;s=s^(s>>15n);s=(s*0x85EBCA6Bn)&0xFFFFFFFFn;s=s^(s>>13n);s=(s*0xC2B2AE35n)&0xFFFFFFFFn;s=s^(s>>16n);xo_table[ix]=Number(s);}}
function srand_get_random(){var t1x5=BigInt(xo_table[1])*5n;t1x5=Number(t1x5&0xFFFFFFFFn);var result=BigInt(((t1x5<<7)|(t1x5>>>(32-7))))*9n;result=Number(result&0xFFFFFFFFn);var t1s9=(xo_table[1]<<9)>>>0;xo_table[2]=(xo_table[2]^xo_table[0])>>>0;xo_table[3]=(xo_table[3]^xo_table[1])>>>0;xo_table[1]=(xo_table[1]^xo_table[2])>>>0;xo_table[0]=(xo_table[0]^xo_table[3])>>>0;xo_table[2]=(xo_table[2]^t1s9)>>>0;var t3=xo_table[3];xo_table[3]=((t3<<11)|(t3>>>(32-11)))>>>0;return result>>>0;}
...
function do_gestalt(val,val2){var ix;switch(val){case 0:return 0x00030103;case 1:return 0x00020202;case 2:return 1;case 3:return 1;case 4:switch(val2){case 0:return 1;case 1:return 1;case 2:return 1;default:return 0;}
...
return false;var chunks=[];chunks.push({key:"IFhd",chunk:game_image.slice(0,128)});var cmem=memmap.slice(ramstart);for(var i=ramstart;i<game_image.length;i++){cmem[i-ramstart]^=game_image[i];}
...
valstack.length=origstacklen;snapshot.stringtable=self.stringtable;snapshot.iosysmode=self.iosysmode;snapshot.iosysrock=self.iosysrock;snapshot.protectstart=self.protectstart;snapshot.protectend=self.protectend;if(self.random_func==xo_random_func&&xo_table){snapshot.xo_table=xo_table.slice(0);}
...
heapend=addr+size;}
if(heapend<self.endmem){freelist.push(new HeapBlock(heapend,self.endmem-heapend));}}
set_string_table(snapshot.stringtable);set_iosys(snapshot.iosysmode,snapshot.iosysrock);self.protectstart=snapshot.protectstart;self.protectend=snapshot.protectend;if(snapshot.xo_table===undefined&&snapshot.srand_table!==undefined){snapshot.xo_table=snapshot.srand_table.slice(0,4);}
if(snapshot.xo_table===undefined){set_random(0);}
else{set_random(1);xo_table=snapshot.xo_table.slice(0);}
...
if(!gameurl&&image){GlkOte.log('GiLoad: trying pre-loaded load ('+image_format+')...');switch(image_format){case'base64':image=decode_base64(image);break;case'raw':image=decode_raw_text(image);break;case'array':break;default:all_options.io.fatal_error("Could not decode story file data: "+image_format);return;}
...
{var data=atob(base64data);var image=Array(data.length);var ix;for(ix=0;ix<data.length;ix++)
