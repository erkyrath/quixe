'use strict';var QuixeClass=function(){var self={};function quixe_prepare(image,all_options){console.log('### quixe_prepare',all_options);self.GiDispa=all_options.GiDispa;self.GiLoad=all_options.GiLoad;self.Glk=all_options.io;game_image=image;var ls=game_image.slice(0,64);var ix,val;for(ix=0;ix<ls.length;ix++){val=ls[ix].toString(16);if(val.length<2)
val="0"+val;ls[ix]=val;}
