'use strict';var Quixe=function(){var self={};function quixe_prepare(image,all_options){self.GiDispa=all_options.GiDispa;game_image=image;var ls=game_image.slice(0,64);var ix,val;for(ix=0;ix<ls.length;ix++){val=ls[ix].toString(16);if(val.length<2)
val="0"+val;ls[ix]=val;}
...
function quixe_getlibrary(val){switch(val){case'GiDispa':return self.GiDispa;case'Glk':return self.Glk;}
return null;}
...
qlog('JS stack dump A:\n'+ex.stack);qstackdump();Glk.fatal_error("Quixe init: "+show_exception(ex));if(true||opt_rethrow_exceptions)
throw ex;}}
...
Glk.update();if(opt_log_execution_time){qlog("event executed in "+(pathend-pathstart)+" ms");}}
return{version:'2.1.8',prepare:quixe_prepare,getlibrary:quixe_getlibrary,init:quixe_init,resume:quixe_resume,get_signature:quixe_get_signature,get_vm_internals:quixe_get_vm_self,get_statistics:quixe_get_statistics,get_debuginfo:quixe_get_debuginfo,ReadByte:ReadArgByte,WriteByte:WriteArgByte,ReadWord:ReadArgWord,WriteWord:WriteArgWord,ReadStructField:ReadStructField,WriteStructField:WriteStructField,SetResumeStore:SetResumeStore,do_autosave:vm_autosave};}();try{exports.Quixe=Quixe;}catch(ex){};'use strict';var GiDispaClass=function(){var self={};self.VM=null;function gidispa_init(options){self.VM=options.VM;console.log('### GiDispa init: selfobj',self);}
...
function init_module(){var ix,key;last_used_id=1+Math.round(Math.random()*1000);for(ix in class_defs){key=class_defs[ix];class_map[key]={};}};init_module();return{init:gidispa_init,inited:gidispa_inited,getlibrary:gidispa_getlibrary,get_function:get_function,prepare_resume:prepare_resume,check_autosave:check_autosave,class_register:class_register,class_unregister:class_unregister,class_obj_to_id:class_obj_to_id,class_obj_from_id:class_obj_from_id,retain_array:retain_array,unretain_array:unretain_array,get_retained_array:get_retained_array};};try{exports.GiDispaClass=GiDispaClass;}catch(ex){};'use strict';var GiLoad=function(){var all_options={vm:null,io:null,spacing:4,use_query_story:true,default_story:null,set_page_title:true,default_page_title:'Game',game_format_name:'',exit_warning:'The game session has ended.',image_info_map:null,proxy_url:'https://zcode.appspot.com/proxy/'};var gameurl=null;var metadata={};var coverimageres=undefined;var debug_info=null;var blorbchunks={};var alttexts={};var GlkOte=null;function load_run(optobj,image,imageoptions){if(!imageoptions){imageoptions={};}
else if(typeof(imageoptions)=='string'){imageoptions={format:imageoptions};}
else{}
var image_format=imageoptions.format;if(!image_format)
image_format='array';all_options.io=window.Glk;all_options.vm=window.Quixe;all_options.GlkOte=new window.GlkOteClass();all_options.GiDispa=new window.GiDispaClass();GlkOte=all_options.GlkOte;if(!optobj)
...
return{load_run:load_run,find_data_chunk:find_data_chunk,get_metadata:get_metadata,get_cover_pict:get_cover_pict,get_debug_info:get_debug_info,get_image_info:get_image_info,get_image_url:get_image_url};}();try{exports.GiLoad=GiLoad;}catch(ex){};