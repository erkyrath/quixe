...
debugmod.open();}}
if(Dialog.init_async){Dialog.init_async(function(){finish_init(iface);})}
else{if(Dialog.init){Dialog.init();}
finish_init(iface);}}
function finish_init(iface){if(!iface.font_load_delay){send_response('init',null,current_metrics);}
...
msg='???';var el=document.getElementById(errorcontent_id);remove_children(el);el.appendChild(document.createTextNode(msg));el=document.getElementById(errorpane_id);if(el.className=='WarningPane')
...
var el=document.getElementById(errorcontent_id);remove_children(el);el.appendChild(document.createTextNode(msg));$('#'+errorpane_id).addClass('WarningPane');$('#'+errorpane_id).show();hide_loading();}
...
return{version:'2.2.6',init:glkote_init,update:glkote_update,extevent:glkote_extevent,getinterface:glkote_get_interface,getdomcontext:glkote_get_dom_context,setdomcontext:glkote_set_dom_context,save_allstate:glkote_save_allstate,log:glkote_log,warning:glkote_warning,error:glkote_error};}();try{exports.GlkOte=GlkOte;}catch(ex){};'use strict';var Dialog=function(){const electron=require('electron');const fs=require('fs');const path_mod=require('path');const buffer_mod=require('buffer');var inited=false;var userpath=null;var temppath=null;var extfilepath=null;const filemode_Write=0x01;const filemode_Read=0x02;const filemode_ReadWrite=0x03;const filemode_WriteAppend=0x05;const seekmode_Start=0;const seekmode_Current=1;const seekmode_End=2;const fileusage_Data=0x00;const fileusage_SavedGame=0x01;const fileusage_Transcript=0x02;const fileusage_InputRecord=0x03;const BUFFER_SIZE=256;function init_async(callback)
...
return{streaming:true,init_async:init_async,open:dialog_open,file_clean_fixed_name:file_clean_fixed_name,file_construct_ref:file_construct_ref,file_construct_temp_ref:file_construct_temp_ref,file_ref_exists:file_ref_exists,file_remove_ref:file_remove_ref,file_fopen:file_fopen,file_write:file_write,file_read:file_read,autosave_write:autosave_write,autosave_read:autosave_read};}();try{exports.Dialog=Dialog;}catch(ex){};'use strict';var Glk=function(){var VM=null;var has_canvas;var option_exit_warning;var option_do_vm_autosave;var option_before_select_hook;var option_extevent_hook;var option_glk_gestalt_hook;var has_exited=false;var ui_disabled=false;var ui_specialinput=null;var ui_specialcallback=null;var event_generation=0;var current_partial_inputs=null;var current_partial_outputs=null;function init(vm_options){has_canvas=(document.createElement('canvas').getContext!=undefined);VM=vm_options.vm;if(window.GiDispa)
