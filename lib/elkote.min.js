...
debugmod.open();}}
if(!iface.font_load_delay){send_response('init',null,current_metrics);}
...
{if(inited)return;var obj=electron.ipcRenderer.sendSync('get_app_paths');userpath=obj.userData;temppath=obj.temp;extfilepath=path_mod.join(userpath,'quixe-files');try{fs.mkdirSync(extfilepath);}
catch(ex){}
inited=true;}
...
{init();const dialog=electron.remote.dialog;var opts={filters:filters_for_usage(usage)};var mainwin=electron.remote.getCurrentWindow();if(!tosave){opts.properties=['openFile'];dialog.showOpenDialog(mainwin,opts).then(function(res){var ls=res.filePaths;if(res.canceled||!ls||!ls.length){callback(null);}
else{var ref={filename:ls[0],usage:usage};callback(ref);}});}
else{dialog.showSaveDialog(mainwin,opts).then(function(res){if(res.canceled||!res.filePath){callback(null);}
else{var ref={filename:res.filePath,usage:usage};callback(ref);}});}}
function file_clean_fixed_name(filename,usage)
{init();var res=filename.replace(/["/\\<>:|?*]/g,'');var pos=res.indexOf('.');if(pos>=0)
...
{init();var timestamp=new Date().getTime();var filename="_quixe_temp_"+timestamp+"_"+Math.random();filename=filename.replace('.','');var path=path_mod.join(temppath,filename);var ref={filename:path,usage:usage};return ref;}
function file_ref_exists(ref)
{init();try{fs.accessSync(ref.filename,fs.F_OK);return true;}
catch(ex){return false;}}
function file_remove_ref(ref)
{init();try{fs.unlinkSync(ref.filename);}
catch(ex){}}
...
{init();var fstream=new FStream(fmode,ref.filename);if(fmode==filemode_ReadWrite||fmode==filemode_WriteAppend){try{var tempfd=fs.openSync(fstream.filename,"a");fs.closeSync(tempfd);}
...
{init();var gamedirpath=path_mod.join(userpath,'games',signature);var stat=null;try{stat=fs.statSync(gamedirpath);}
...
{init();var gamedirpath=path_mod.join(userpath,'games',signature);var pathj=path_mod.join(gamedirpath,'autosave.json');var pathr=path_mod.join(gamedirpath,'autosave.ram');try{var str=fs.readFileSync(pathj,{encoding:'utf8'});var snapshot=JSON.parse(str);try{var buf=fs.readFileSync(pathr,{encoding:null});var ram=Array.from(buf.values());snapshot.ram=ram;}
