...
function create_resize_sensors(){const gameport=$('#'+gameport_id,dom_context);if(!gameport.length)
return'Cannot find gameport element #'+gameport_id+' in this document.';const shrinkel=$('<div>',{id:dom_prefix+'resize-sensor-shrink'}).css({position:'absolute',left:'0',right:'0',top:'0',bottom:'0',overflow:'hidden',visibility:'hidden','z-index':'-1'});shrinkel.append($('<div>',{id:dom_prefix+'resize-sensor-shrink-child'}).css({position:'absolute',left:'0',right:'0',width:'200%',height:'200%'}));const expandel=$('<div>',{id:dom_prefix+'resize-sensor-expand'}).css({position:'absolute',left:'0',right:'0',top:'0',bottom:'0',overflow:'hidden',visibility:'hidden','z-index':'-1'});expandel.append($('<div>',{id:dom_prefix+'resize-sensor-expand-child'}).css({position:'absolute',left:'0',right:'0'}));const shrinkdom=shrinkel.get(0);const expanddom=expandel.get(0);const expandchilddom=expanddom.childNodes[0];function reset(){shrinkdom.scrollLeft=100000;shrinkdom.scrollTop=100000;expandchilddom.style.width='100000px';expandchilddom.style.height='100000px';expanddom.scrollLeft=100000;expanddom.scrollTop=100000;}
gameport.append(shrinkel);gameport.append(expandel);reset();function evhan(ev){evhan_doc_resize(ev);reset();}
...
if(autorestore.recording_sessionid){if(recording&&recording_state){recording_state.sessionId=autorestore.recording_sessionid;}}
