...
$(document).on('keypress',evhan_doc_keypress);$(window).on('resize',evhan_doc_resize);current_devpixelratio=window.devicePixelRatio||1;window.matchMedia('screen and (min-resolution: 1.5dppx)').addListener(evhan_doc_pixelreschange);window.matchMedia('screen and (min-resolution: 2dppx)').addListener(evhan_doc_pixelreschange);window.matchMedia('screen and (min-resolution: 3dppx)').addListener(evhan_doc_pixelreschange);window.matchMedia('screen and (min-resolution: 4dppx)').addListener(evhan_doc_pixelreschange);var res=measure_window();if(jQuery.type(res)==='string'){glkote_error(res);return;}
...
moreel.remove();}
else{if(!moreel.length){moreel=$('<div>',{id:'win'+win.id+'_moreprompt','class':'MorePrompt'});moreel.append('More');var morex=win.coords.right+approx_scroll_width;var morey=win.coords.bottom;moreel.css({bottom:morey+'px',right:morex+'px'});$('#'+windowport_id,dom_context).append(moreel);}}}}});readjust_paging_focus(false);disabled=false;if(arg.disable||arg.specialinput){disabled=true;jQuery.each(windowdic,function(winid,win){if(win.inputel){win.inputel.prop('disabled',true);}});}
...
win.scaleratio=current_devpixelratio/win.backpixelratio;glkote_log('### created canvas with scale '+win.scaleratio+' (device '+current_devpixelratio+' / backstore '+win.backpixelratio+')');el.attr('width',win.graphwidth*win.scaleratio);el.attr('height',win.graphheight*win.scaleratio);el.css('width',(win.graphwidth+'px'));el.css('height',(win.graphheight+'px'));win.frameel.css('background-color',win.defcolor);if(ctx){ctx.setTransform(win.scaleratio,0,0,win.scaleratio,0,0);}
...
var canvas=undefined;var el=$('#win'+win.id+'_canvas',dom_context);var ctx=canvas_get_2dcontext(el);if(!ctx){glkote_log('perform_graphics_ops: op for nonexistent canvas '+win.id);graphics_draw_queue.shift();continue;}
...
function evhan_doc_pixelreschange(ev){var ratio=window.devicePixelRatio||1;if(ratio!=current_devpixelratio){current_devpixelratio=ratio;glkote_log('### devicePixelRatio changed to '+current_devpixelratio);jQuery.each(windowdic,function(winid,win){if(win.type=='graphics'){var el=$('#win'+win.id+'_canvas',dom_context);win.scaleratio=current_devpixelratio/win.backpixelratio;glkote_log('### changed canvas to scale '+win.scaleratio+' (device '+current_devpixelratio+' / backstore '+win.backpixelratio+')');var ctx=canvas_get_2dcontext(el);el.attr('width',win.graphwidth*win.scaleratio);el.attr('height',win.graphheight*win.scaleratio);el.css('width',(win.graphwidth+'px'));el.css('height',(win.graphheight+'px'));if(ctx){ctx.setTransform(win.scaleratio,0,0,win.scaleratio,0,0);ctx.fillStyle=win.defcolor;ctx.fillRect(0,0,win.graphwidth,win.graphheight);ctx.fillStyle='#000000';}
...
function window_scroll_to_bottom(win){var frameel=win.frameel;var frameheight=frameel.outerHeight();frameel.scrollTop(frameel.get(0).scrollHeight-frameheight);var realbottom=last_line_top_offset(frameel);var newtopunseen=frameel.scrollTop()+frameheight;if(newtopunseen>realbottom)
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(win.needspaging){if(frameel.scrollTop()+frameheight+moreprompt_margin>=frameel.get(0).scrollHeight){win.needspaging=false;var moreel=$('#win'+win.id+'_moreprompt',dom_context);if(moreel.length)
moreel.remove();readjust_paging_focus(true);}}}
...
return{version:'2.2.1',init:glkote_init,update:glkote_update,extevent:glkote_extevent,getinterface:glkote_get_interface,getdomcontext:glkote_get_dom_context,setdomcontext:glkote_set_dom_context,save_allstate:glkote_save_allstate,log:glkote_log,warning:glkote_warning,error:glkote_error};}();Dialog=function(){const fs=require('fs');const path_mod=require('path');const buffer_mod=require('buffer');var userpath=require('electron').remote.app.getPath('userData');var extfilepath=path_mod.join(userpath,'quixe-files');try{fs.mkdirSync(extfilepath);}
catch(ex){}
const filemode_Write=0x01;const filemode_Read=0x02;const filemode_ReadWrite=0x03;const filemode_WriteAppend=0x05;const seekmode_Start=0;const seekmode_Current=1;const seekmode_End=2;const fileusage_Data=0x00;const fileusage_SavedGame=0x01;const fileusage_Transcript=0x02;const fileusage_InputRecord=0x03;const BUFFER_SIZE=256;function filters_for_usage(val)
...
{const dialog=require('electron').remote.dialog;var opts={filters:filters_for_usage(usage)};var mainwin=require('electron').remote.getCurrentWindow();if(!tosave){opts.properties=['openFile'];dialog.showOpenDialog(mainwin,opts,function(ls){if(!ls||!ls.length){callback(null);}
else{var ref={filename:ls[0],usage:usage};callback(ref);}});}
else{dialog.showSaveDialog(mainwin,opts,function(path){if(!path){callback(null);}
else{var ref={filename:path,usage:usage};callback(ref);}});}}
function file_clean_fixed_name(filename,usage){var res=filename.replace(/["/\\<>:|?*]/g,'');var pos=res.indexOf('.');if(pos>=0)
...
{var timestamp=new Date().getTime();var filename="_temp_"+timestamp+"_"+Math.random();filename=filename.replace('.','');var temppath=require('electron').remote.app.getPath('temp');var path=path_mod.join(temppath,filename);var ref={filename:path,usage:usage};return ref;}
...
{this.fmode=fmode;this.filename=filename;this.fd=null;this.mark=0;this.buffer=new buffer_mod.Buffer(BUFFER_SIZE);this.bufuse=0;this.buflen=0;this.bufmark=0;}
...
var ram=snapshot.ram;snapshot.ram=undefined;var str=JSON.stringify(snapshot);fs.writeFileSync(pathj,str,{encoding:'utf8'});var buf=new buffer_mod.Buffer(ram);fs.writeFileSync(pathr,buf);}
function autosave_read(signature)
{var gamedirpath=path_mod.join(userpath,'games',signature);var pathj=path_mod.join(gamedirpath,'autosave.json');var pathr=path_mod.join(gamedirpath,'autosave.ram');try{var str=fs.readFileSync(pathj,{encoding:'utf8'});var snapshot=JSON.parse(str);var buf=fs.readFileSync(pathr,{encoding:null});var ram=Array.from(buf.values());snapshot.ram=ram;return snapshot;}
