'use strict';var GlkOteClass=function(){var is_inited=false;var game_interface=null;var dom_context=undefined;var dom_prefix='';var windowport_id='windowport';var gameport_id='gameport';var errorpane_id='errorpane';var errorcontent_id='errorcontent';var loadingpane_id='loadingpane';var max_buffer_length=800;var generation=0;var generation_sent=-1;var disabled=false;var loading_visible=null;var error_visible=false;var windowdic=null;var current_metrics=null;var current_devpixelratio=null;var currently_focussed=false;var last_known_focus=0;var last_known_paging=0;var windows_paging_count=0;var graphics_draw_queue=[];var request_timer=null;var request_timer_interval=null;var resize_timer=null;var retry_timer=null;var perform_paging=true;var detect_external_links=false;var regex_external_links=null;var debug_out_handler=null;var Dialog=null;var Blorb=null;var NBSP="\xa0";var approx_scroll_width=20;var moreprompt_margin=4;var key_codes={KEY_BACKSPACE:8,KEY_TAB:9,KEY_RETURN:13,KEY_ESC:27,KEY_LEFT:37,KEY_UP:38,KEY_RIGHT:39,KEY_DOWN:40,KEY_DELETE:46,KEY_HOME:36,KEY_END:35,KEY_PAGEUP:33,KEY_PAGEDOWN:34,KEY_INSERT:45};var terminator_key_names={escape:key_codes.KEY_ESC,func1:112,func2:113,func3:114,func4:115,func5:116,func6:117,func7:118,func8:119,func9:120,func10:121,func11:122,func12:123};var terminator_key_values={};var recording=false;var recording_state=null;var recording_handler=null;var recording_handler_url=null;var recording_context={};var image_cache={};function glkote_init(iface){if(!iface&&window.Game)
...
loadingpane_id=iface.loadingpane;var el=$('#'+windowport_id,dom_context);if(!el.length){glkote_error('Cannot find windowport element #'+windowport_id+' in this document.');return;}
...
if(iface.debug_commands){var debugmod=window.GiDebug;if(iface.debug_commands!=true)
...
if(Dialog&&!Dialog.inited()){var dialogiface={GlkOte:this};if(iface.dialog_dom_prefix){dialogiface.dom_prefix=iface.dialog_dom_prefix;}
if(Dialog.init_async){Dialog.init_async(dialogiface,function(){finish_init(iface);})
return;}
...
function measure_window(){var metrics={};var winsize,line1size,line2size,invcursize,spansize,canvassize;var gameport=$('#'+gameport_id,dom_context);if(!gameport.length)
return'Cannot find gameport element #'+gameport_id+' in this document.';$('#'+dom_prefix+'layouttestpane',dom_context).remove();metrics.width=gameport.width();metrics.height=gameport.height();var layout_test_pane=$('<div>',{id:dom_prefix+'layout_test_pane'});layout_test_pane.text('This should not be visible');layout_test_pane.css({position:'absolute',visibility:'hidden',left:'-1000px'});var line=$('<div>');line.append($('<span>',{'class':"Style_normal"}).text('12345678'));var gridwin=$('<div>',{'class':'WindowFrame GridWindow'});var gridline1=line.clone().addClass('GridLine').appendTo(gridwin);var gridline2=line.clone().addClass('GridLine').appendTo(gridwin);var gridspan=gridline1.children('span');layout_test_pane.append(gridwin);var bufwin=$('<div>',{'class':'WindowFrame BufferWindow'});var bufline1=line.clone().addClass('BufferLine').appendTo(bufwin);var bufline2=line.clone().addClass('BufferLine').appendTo(bufwin);var invcurspan=$('<span>',{'class':'InvisibleCursor'});bufline2.append(invcurspan);var bufspan=bufline1.children('span');layout_test_pane.append(bufwin);var graphwin=$('<div>',{'class':'WindowFrame GraphicsWindow'});var graphcanvas=$('<canvas>');graphcanvas.attr('width',64);graphcanvas.attr('height',32);graphwin.append(graphcanvas);layout_test_pane.append(graphwin);gameport.append(layout_test_pane);var get_size=function(el){return{width:el.outerWidth(),height:el.outerHeight()};};winsize=get_size(gridwin);spansize=get_size(gridspan);line1size=get_size(gridline1);line2size=get_size(gridline2);metrics.gridcharheight=Math.max(1,gridline2.position().top-gridline1.position().top);metrics.gridcharwidth=Math.max(1,gridspan.width()/8);metrics.gridmarginx=winsize.width-spansize.width;metrics.gridmarginy=winsize.height-(line1size.height+line2size.height);winsize=get_size(bufwin);spansize=get_size(bufspan);line1size=get_size(bufline1);line2size=get_size(bufline2);invcursize=get_size(invcurspan);metrics.buffercharheight=Math.max(1,bufline2.position().top-bufline1.position().top);metrics.buffercharwidth=Math.max(1,bufspan.width()/8);metrics.buffermarginx=winsize.width-spansize.width;metrics.buffermarginy=winsize.height-(line1size.height+line2size.height)+2;winsize=get_size(graphwin);canvassize=get_size(graphcanvas);metrics.graphicsmarginx=winsize.width-canvassize.width;metrics.graphicsmarginy=winsize.height-canvassize.height;layout_test_pane.remove();metrics.outspacingx=0;metrics.outspacingy=0;metrics.inspacingx=0;metrics.inspacingy=0;if(game_interface.spacing!=undefined){metrics.outspacingx=game_interface.spacing;metrics.outspacingy=game_interface.spacing;metrics.inspacingx=game_interface.spacing;metrics.inspacingy=game_interface.spacing;}
...
return'Cannot find gameport element #'+gameport_id+' in this document.';var shrinkel=$('<div>',{id:dom_prefix+'resize-sensor-shrink'}).css({position:'absolute',left:'0',right:'0',top:'0',bottom:'0',overflow:'hidden',visibility:'hidden','z-index':'-1'});shrinkel.append($('<div>',{id:dom_prefix+'resize-sensor-shrink-child'}).css({position:'absolute',left:'0',right:'0',width:'200%',height:'200%'}));var expandel=$('<div>',{id:dom_prefix+'resize-sensor-expand'}).css({position:'absolute',left:'0',right:'0',top:'0',bottom:'0',overflow:'hidden',visibility:'hidden','z-index':'-1'});expandel.append($('<div>',{id:dom_prefix+'resize-sensor-expand-child'}).css({position:'absolute',left:'0',right:'0'}));var shrinkdom=shrinkel.get(0);var expanddom=expandel.get(0);var expandchilddom=expanddom.childNodes[0];var reset=function(){shrinkdom.scrollLeft=100000;shrinkdom.scrollTop=100000;expandchilddom.style.width='100000px';expandchilddom.style.height='100000px';expanddom.scrollLeft=100000;expanddom.scrollTop=100000;}
...
accept_specialinput(arg.specialinput);for(const win of windowdic.values()){if(win.type=='buffer'&&win.needscroll){win.needscroll=false;if(!win.needspaging){var frameel=win.frameel;if(!perform_paging){frameel.scrollTop(frameel.get(0).scrollHeight);win.needspaging=false;}
...
var moreel=$('#'+dom_prefix+'win'+win.id+'_moreprompt',dom_context);var prevel=$('#'+dom_prefix+'win'+win.id+'_prevmark',dom_context);if(!win.needspaging){if(moreel.length)
moreel.remove();if(prevel.length)
prevel.remove();}
else{if(!moreel.length){moreel=$('<div>',{id:dom_prefix+'win'+win.id+'_moreprompt','class':'MorePrompt'});moreel.append('More');var morex=win.coords.right+approx_scroll_width;var morey=win.coords.bottom;moreel.css({bottom:morey+'px',right:morex+'px'});$('#'+windowport_id,dom_context).append(moreel);}
...
var newinputwin=0;if(!disabled&&!windows_paging_count){for(const win of windowdic.values()){if(win.input){if(!newinputwin||win.id==last_known_focus)
newinputwin=win.id;}}}
if(newinputwin){var focusfunc=function(){var win=windowdic.get(newinputwin);if(win.inputel){win.inputel.focus();}};defer_func(focusfunc);}
if(autorestore){if(autorestore.history){for(const[winid,ls]of Object.entries(autorestore.history)){var win=windowdic.get(winid);if(win!=null){win.history=ls.slice(0);win.historypos=win.history.length;}}}
if(autorestore.defcolor){for(const[winid,val]of Object.entries(autorestore.defcolor)){var win=windowdic.get(winid);if(win!=null){win.defcolor=val;}}}
...
arg.forEach(accept_one_window);var closewins=[];for(const win of windowdic.values()){if(!win.inplace){closewins.push(win);}}
...
typeclass='GraphicsWindow';var rockclass='WindowRock_'+arg.rock;frameel=$('<div>',{id:dom_prefix+'window'+arg.id,'class':'WindowFrame HasNoInputField '+typeclass+' '+rockclass});frameel.data('winid',arg.id);frameel.on('mousedown',arg.id,evhan_window_mousedown);if(perform_paging&&win.type=='buffer')
...
win.inplace=true;if(win.type=='grid'){var ix;if(arg.gridheight>win.gridheight){for(ix=win.gridheight;ix<arg.gridheight;ix++){var el=$('<div>',{id:dom_prefix+'win'+win.id+'_ln'+ix,'class':'GridLine'});el.append(NBSP);win.frameel.append(el);}}
if(arg.gridheight<win.gridheight){for(ix=arg.gridheight;ix<win.gridheight;ix++){var el=$('#'+dom_prefix+'win'+win.id+'_ln'+ix,dom_context);if(el.length)
el.remove();}}
win.gridheight=arg.gridheight;win.gridwidth=arg.gridwidth;}
if(win.type=='buffer'){}
if(win.type=='graphics'){var el=$('#'+dom_prefix+'win'+win.id+'_canvas',dom_context);if(!el.length){win.graphwidth=arg.graphwidth;win.graphheight=arg.graphheight;win.defcolor='#FFF';el=$('<canvas>',{id:dom_prefix+'win'+win.id+'_canvas'});win.backpixelratio=1;var canvas=el.get(0);var ctx=canvas_get_2dcontext(el);if(ctx){win.backpixelratio=ctx.webkitBackingStorePixelRatio||ctx.mozBackingStorePixelRatio||ctx.msBackingStorePixelRatio||ctx.oBackingStorePixelRatio||ctx.backingStorePixelRatio||1;}
...
function close_one_window(win){win.frameel.remove();windowdic.delete(win.id);win.frameel=null;var moreel=$('#'+dom_prefix+'win'+win.id+'_moreprompt',dom_context);if(moreel.length)
moreel.remove();}
...
var cursel=$('#'+dom_prefix+'win'+win.id+'_cursor',dom_context);if(cursel.length)
...
for(sx=0;sx<content.length;sx++){var rdesc=content[sx];var rstyle,rtext,rlink;if(jQuery.type(rdesc)==='object'){if(rdesc.special!==undefined){if(rdesc.special=='image'){var imgurl=rdesc.url;if(Blorb&&Blorb.get_image_url){var newurl=Blorb.get_image_url(rdesc.image);if(newurl)
imgurl=newurl;}
...
var divel=buffer_last_line(win);if(divel){var cursel=$('<span>',{id:dom_prefix+'win'+win.id+'_cursor','class':'InvisibleCursor'});divel.append(cursel);if(win.inputel){var inputel=win.inputel;var pos=cursel.position();var width=win.frameel.width()-(current_metrics.buffermarginx+pos.left+2);if(width<1)
...
draw=[];var docall=(graphics_draw_queue.length==0);for(ix=0;ix<draw.length;ix++){var op=draw[ix];var newop={winid:win.id};Object.assign(newop,op);graphics_draw_queue.push(newop);}
if(docall&&graphics_draw_queue.length>0){perform_graphics_ops(null);}}}
function accept_inputcancel(arg){var hasinput={};for(const argi of arg){if(argi.type)
hasinput[argi.id]=argi;}
for(const win of windowdic.values()){if(win.input){var argi=hasinput[win.id];if(argi==null||argi.gen>win.input.gen){win.input=null;win.frameel.addClass('HasNoInputField');win.frameel.removeClass('HasInputField');if(win.inputel){win.inputel.remove();win.inputel=null;}}}}}
function accept_inputset(arg){var hasinput={};var hashyperlink={};var hasmouse={};for(const argi of arg){if(argi.type)
hasinput[argi.id]=argi;if(argi.hyperlink)
hashyperlink[argi.id]=true;if(argi.mouse)
hasmouse[argi.id]=true;}
for(const win of windowdic.values()){win.reqhyperlink=hashyperlink[win.id];win.reqmouse=hasmouse[win.id];var argi=hasinput[win.id];if(argi==null)
continue;win.input=argi;win.frameel.addClass('HasInputField');win.frameel.removeClass('HasNoInputField');var maxlen=1;if(argi.type=='line')
maxlen=argi.maxlen;var newinputel=false;var inputel=win.inputel;if(inputel==null){newinputel=true;var classes='Input';if(argi.type=='line'){classes+=' LineInput';}
else if(argi.type=='char'){classes+=' CharInput';}
else{glkote_error('Window '+win.id+' has requested unrecognized input type '+argi.type+'.');}
inputel=$('<input>',{id:dom_prefix+'win'+win.id+'_input','class':classes,type:'text',maxlength:maxlen});if(true)
...
if(win.type=='grid'){var lineel=$('#'+dom_prefix+'win'+win.id+'_ln'+argi.ypos,dom_context);if(!lineel.length){glkote_error('Window '+win.id+' has requested input at unknown line '+argi.ypos+'.');return;}
...
if(win.type=='buffer'){var cursel=$('#'+dom_prefix+'win'+win.id+'_cursor',dom_context);if(!cursel.length){cursel=$('<span>',{id:dom_prefix+'win'+win.id+'_cursor','class':'InvisibleCursor'});win.frameel.append(cursel);}
...
function readjust_paging_focus(canfocus){windows_paging_count=0;var pageable_win=0;if(perform_paging){for(const win of windowdic.values()){if(win.needspaging){windows_paging_count+=1;if(!pageable_win||win.id==last_known_paging)
pageable_win=win.id;}}}
if(windows_paging_count){last_known_paging=pageable_win;}
if(!windows_paging_count&&canfocus){var newinputwin=0;if(!disabled&&!windows_paging_count){for(const win of windowdic.values()){if(win.input){if(!newinputwin||win.id==last_known_focus)
newinputwin=win.id;}}}
...
function glkote_save_allstate(){var obj={metrics:{width:current_metrics.width,height:current_metrics.height},history:{}};for(const[winid,win]of windowdic.entries()){if(win.history&&win.history.length)
...
msg='???';var el=document.getElementById(errorcontent_id);if(!el)return;remove_children(el);el.appendChild(document.createTextNode(msg));el=document.getElementById(errorpane_id);if(el.className=='WarningPane')
...
var el=document.getElementById(errorcontent_id);if(!el)return;remove_children(el);el.appendChild(document.createTextNode(msg));$('#'+errorpane_id).addClass('WarningPane');$('#'+errorpane_id).show();hide_loading();}
...
function clear_error(){$('#'+errorpane_id,dom_context).hide();}
function hide_loading(){if(loading_visible==false)
return;loading_visible=false;var el=document.getElementById(loadingpane_id);if(el){el.style.display='none';}}
function show_loading(){if(loading_visible==true)
return;loading_visible=true;var el=document.getElementById(loadingpane_id);if(el){el.style.display='';}}
...
while(graphics_draw_queue.length){var op=graphics_draw_queue[0];var win=windowdic.get(op.winid);if(!win){glkote_log('perform_graphics_ops: op for nonexistent window '+op.winid);graphics_draw_queue.shift();continue;}
var el=$('#'+dom_prefix+'win'+win.id+'_canvas',dom_context);var ctx=canvas_get_2dcontext(el);if(!ctx){glkote_log('perform_graphics_ops: op for nonexistent canvas '+win.id);graphics_draw_queue.shift();continue;}
...
if(!loadedimg){var imgurl=op.url;if(Blorb&&Blorb.get_image_url){var newurl=Blorb.get_image_url(op.image);if(newurl)
imgurl=newurl;}
...
function inspect_deep(res){const keys=res.keys().sort();const els=keys.map(key=>{var val=res[key];if(jQuery.type(val)==='string')
val="'"+val+"'";else if(!(jQuery.type(val)==='number'))
val=inspect_deep(val);return key+':'+val;});return'{'+els.join(', ')+'}';}
function inspect_shallow(res){const keys=res.keys().sort();const els=keys.map(key=>{var val=res[key];if(jQuery.type(val)==='string')
...
var winid=0;if(win)
winid=win.id;var res={type:type,gen:generation};generation_sent=generation;if(type=='line'){res.window=win.id;res.value=val;if(val2)
res.terminator=val2;}
...
if(!(type=='init'||type=='refresh'||type=='specialresponse'||type=='debuginput')){for(const win of windowdic.values()){var savepartial=(type!='line'&&type!='char')||(win.id!=winid);if(savepartial&&win.input&&win.input.type=='line'&&win.inputel&&win.inputel.val()){var partial=res.partial;if(!partial){partial={};res.partial=partial;};partial[win.id]=win.inputel.val();}}}
...
var new_metrics=measure_window();if(metrics_match(new_metrics,current_metrics)){return;}
current_metrics=new_metrics;send_response('arrange',null,current_metrics);}
function send_window_redraw(winid){var win=windowdic.get(winid);if(!win||win.type!='graphics')
return;send_response('redraw',win,null);}
function evhan_doc_pixelreschange(ev){var ratio=window.devicePixelRatio||1;if(ratio!=current_devpixelratio){current_devpixelratio=ratio;for(const[winid,win]of windowdic.entries()){if(win.type=='graphics'){var el=$('#'+dom_prefix+'win'+win.id+'_canvas',dom_context);win.scaleratio=current_devpixelratio/win.backpixelratio;var ctx=canvas_get_2dcontext(el);el.attr('width',win.graphwidth*win.scaleratio);el.attr('height',win.graphheight*win.scaleratio);el.css('width',(win.graphwidth+'px'));el.css('height',(win.graphheight+'px'));if(ctx){ctx.setTransform(win.scaleratio,0,0,win.scaleratio,0,0);ctx.fillStyle=win.defcolor;ctx.fillRect(0,0,win.graphwidth,win.graphheight);ctx.fillStyle='#000000';}
...
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(win.needspaging){if(frameel.scrollTop()+frameheight+moreprompt_margin>=frameel.get(0).scrollHeight){win.needspaging=false;var moreel=$('#'+dom_prefix+'win'+win.id+'_moreprompt',dom_context);if(moreel.length)
...
ev.preventDefault();return;}}
function evhan_window_mousedown(ev){var winid=ev.data;var win=windowdic.get(winid);if(!win)
return;if(win.inputel){last_known_focus=win.id;}
if(win.needspaging)
last_known_paging=win.id;else if(win.inputel)
last_known_paging=0;}
function evhan_input_mouse_click(ev){var winid=ev.data;var win=windowdic.get(winid);if(!win)
return;if(ev.button!=0)
return;if(!win.reqmouse)
return;var xpos=0;var ypos=0;if(win.type=='grid'){var lineel=$('#'+dom_prefix+'win'+win.id+'_ln'+0,dom_context);if(lineel.length){var linepos=lineel.offset();xpos=Math.floor((ev.clientX-linepos.left)/current_metrics.gridcharwidth);ypos=Math.floor((ev.clientY-linepos.top)/current_metrics.gridcharheight);}
if(xpos>=win.gridwidth)
xpos=win.gridwidth-1;if(xpos<0)
xpos=0;if(ypos>=win.gridheight)
ypos=win.gridheight-1;if(ypos<0)
ypos=0;}
else if(win.type=='graphics'){var canel=$('#'+dom_prefix+'win'+win.id+'_canvas',dom_context);if(canel.length){var pos=canel.offset();xpos=ev.clientX-pos.left;ypos=ev.clientY-pos.top;}
...
if(res){var winid=$(this).data('winid');var win=windowdic.get(winid);if(!win||!win.input)
...
res='return';else
res=String.fromCharCode(keycode);var winid=$(this).data('winid');var win=windowdic.get(winid);if(!win||!win.input)
return true;send_response('char',win,res);return false;}
function evhan_input_keydown(ev){var keycode=0;if(ev)keycode=ev.keyCode;if(!keycode)return true;if(keycode==key_codes.KEY_UP||keycode==key_codes.KEY_DOWN){var winid=$(this).data('winid');var win=windowdic.get(winid);if(!win||!win.input)
...
this.value='';}
return false;}
else if(terminator_key_values[keycode]){var winid=$(this).data('winid');var win=windowdic.get(winid);if(!win||!win.input)
...
return true;}
function evhan_input_keypress(ev){var keycode=0;if(ev)keycode=ev.which;if(!keycode)return true;if(keycode==13){var winid=$(this).data('winid');var win=windowdic.get(winid);if(!win||!win.input)
return true;submit_line_input(win,this.value,null);return false;}
return true;}
function evhan_input_focus(ev){var winid=ev.data;var win=windowdic.get(winid);if(!win)
return;currently_focussed=true;last_known_focus=winid;last_known_paging=winid;}
function evhan_input_blur(ev){var winid=ev.data;var win=windowdic.get(winid);if(!win)
return;currently_focussed=false;}
function evhan_window_scroll(ev){var winid=ev.data;var win=windowdic.get(winid);if(!win)
...
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(frameel.scrollTop()+frameheight+moreprompt_margin>=frameel.get(0).scrollHeight){win.needspaging=false;var moreel=$('#'+dom_prefix+'win'+win.id+'_moreprompt',dom_context);if(moreel.length)
...
return{classname:'GlkOte',version:'2.3.0',init:glkote_init,inited:glkote_inited,update:glkote_update,extevent:glkote_extevent,getinterface:glkote_get_interface,getlibrary:glkote_get_library,getdomid:glkote_get_dom_id,getdomcontext:glkote_get_dom_context,setdomcontext:glkote_set_dom_context,save_allstate:glkote_save_allstate,log:glkote_log,warning:glkote_warning,error:glkote_error};};var GlkOte=new GlkOteClass();try{exports.GlkOte=GlkOte;exports.GlkOteClass=GlkOteClass;}catch(ex){};'use strict';var DialogClass=function(){const electron=require('electron');const fs=require('fs');const path_mod=require('path');const buffer_mod=require('buffer');var inited=false;var userpath=null;var temppath=null;var extfilepath=null;var GlkOte=null;const filemode_Write=0x01;const filemode_Read=0x02;const filemode_ReadWrite=0x03;const filemode_WriteAppend=0x05;const seekmode_Start=0;const seekmode_Current=1;const seekmode_End=2;const fileusage_Data=0x00;const fileusage_SavedGame=0x01;const fileusage_Transcript=0x02;const fileusage_InputRecord=0x03;const BUFFER_SIZE=256;function init_async(iface,callback)
{if(inited){callback();return;}
if(iface&&iface.GlkOte){GlkOte=iface.GlkOte;}
if(!GlkOte){throw new Error('ElectroFS: no GlkOte interface!');}
electron.ipcRenderer.invoke('get_app_paths').then(function(obj){userpath=obj.userData;temppath=obj.temp;extfilepath=path_mod.join(userpath,'quixe-files');try{fs.mkdirSync(extfilepath);}
catch(ex){}
inited=true;callback();});}
function dialog_inited(){return inited;}
function dialog_get_library(val){switch(val){case'GlkOte':return GlkOte;}
return null;}
function filters_for_usage(val)
{switch(val){case'data':return[{name:'Glk Data File',extensions:['glkdata']}];case'save':return[{name:'Glk Save File',extensions:['glksave']}];case'transcript':return[{name:'Transcript File',extensions:['txt']}];case'command':return[{name:'Command File',extensions:['txt']}];default:return[];}}
function dialog_open(tosave,usage,gameid,callback)
{var opts={filters:filters_for_usage(usage)};if(!tosave){opts.properties=['openFile'];}
if(!tosave){electron.ipcRenderer.invoke('dialog_open',tosave,opts).then(function(res){var ls=res.filePaths;if(res.canceled||!ls||!ls.length){callback(null);}
else{var ref={filename:ls[0],usage:usage};callback(ref);}});}
else{electron.ipcRenderer.invoke('dialog_open',tosave,opts).then(function(res){if(res.canceled||!res.filePath){callback(null);}
else{var ref={filename:res.filePath,usage:usage};callback(ref);}});}}
function file_clean_fixed_name(filename,usage)
{var res=filename.replace(/["/\\<>:|?*]/g,'');var pos=res.indexOf('.');if(pos>=0)
res=res.slice(0,pos);if(res.length==0)
res="null";switch(usage){case fileusage_Data:return res+'.glkdata';case fileusage_SavedGame:return res+'.glksave';case fileusage_Transcript:case fileusage_InputRecord:return res+'.txt';default:return res;}}
function file_construct_ref(filename,usage,gameid)
{if(!filename)
filename='';if(!usage)
usage='';if(!gameid)
gameid='';var path=path_mod.join(extfilepath,filename);var ref={filename:path,usage:usage};return ref;}
function file_construct_temp_ref(usage)
{var timestamp=new Date().getTime();var filename="_quixe_temp_"+timestamp+"_"+Math.random();filename=filename.replace('.','');var path=path_mod.join(temppath,filename);var ref={filename:path,usage:usage};return ref;}
function file_ref_exists(ref)
{try{fs.accessSync(ref.filename,fs.F_OK);return true;}
catch(ex){return false;}}
function file_remove_ref(ref)
{try{fs.unlinkSync(ref.filename);}
catch(ex){}}
function FStream(fmode,filename)
{this.fmode=fmode;this.filename=filename;this.fd=null;this.mark=0;this.buffer=buffer_mod.Buffer.alloc(BUFFER_SIZE);this.bufuse=0;this.buflen=0;this.bufmark=0;}
FStream.prototype={BufferClass:buffer_mod.Buffer,fclose:function(){if(this.fd===null){GlkOte.log('file_fclose: file already closed: '+this.filename);return;}
this.fflush();fs.closeSync(this.fd);this.fd=null;this.buffer=null;},fread:function(buf,len){if(len===undefined)
len=buf.length;var got=0;while(true){if(this.bufuse==filemode_Read){if(this.bufmark<this.buflen){var want=len-got;if(want>this.buflen-this.bufmark)
want=this.buflen-this.bufmark;if(want>0){this.buffer.copy(buf,got,this.bufmark,this.bufmark+want);this.bufmark+=want;got+=want;}}
if(got>=len)
return got;}
if(this.bufuse)
this.fflush();this.bufuse=filemode_Read;this.bufmark=0;this.buflen=fs.readSync(this.fd,this.buffer,0,BUFFER_SIZE,this.mark);if(this.buflen==0){this.bufuse=0;return got;}}},fwrite:function(buf,len){if(len===undefined)
len=buf.length;var from=0;while(true){if(this.bufuse==filemode_Write){var want=len-from;if(want>BUFFER_SIZE-this.buflen)
want=BUFFER_SIZE-this.buflen;if(want>0){buf.copy(this.buffer,this.buflen,from,from+want);this.buflen+=want;from+=want;}}
if(from>=len)
return from;if(this.bufuse)
this.fflush();this.bufuse=filemode_Write;this.buflen=0;}},ftell:function(){if(this.bufuse==filemode_Read){return this.mark+this.bufmark;}
else if(this.bufuse==filemode_Write){return this.mark+this.buflen;}
else{return this.mark;}},fseek:function(pos,seekmode){this.fflush();var val=0;if(seekmode==seekmode_Current){val=this.mark+pos;}
else if(seekmode==seekmode_End){try{var stats=fs.fstatSync(this.fd);val=stats.size+pos;}
catch(ex){val=this.mark+pos;}}
else{val=pos;}
if(val<0)
val=0;this.mark=val;},fflush:function(){if(this.bufuse==filemode_Read){this.mark+=this.bufmark;}
else if(this.bufuse==filemode_Write){if(this.buflen){var count=fs.writeSync(this.fd,this.buffer,0,this.buflen,this.mark);this.mark+=count;}}
this.bufuse=0;this.buflen=0;this.bufmark=0;}};function file_fopen(fmode,ref)
{var fstream=new FStream(fmode,ref.filename);if(fmode==filemode_ReadWrite||fmode==filemode_WriteAppend){try{var tempfd=fs.openSync(fstream.filename,"a");fs.closeSync(tempfd);}
catch(ex){GlkOte.log('file_fopen: failed to open '+fstream.filename+': '+ex);return null;}}
var modestr=null;switch(fmode){case filemode_Write:modestr="w";break;case filemode_Read:modestr="r";break;case filemode_ReadWrite:modestr="r+";break;case filemode_WriteAppend:modestr="r+";break;}
try{fstream.fd=fs.openSync(fstream.filename,modestr);}
catch(ex){GlkOte.log('file_fopen: failed to open '+fstream.filename+': '+ex);return null;}
if(fmode==filemode_WriteAppend){try{var stats=fs.fstatSync(fstream.fd);fstream.mark=stats.size;}
catch(ex){}}
return fstream;}
function autosave_write(signature,snapshot)
{var gamedirpath=path_mod.join(userpath,'games',signature);var stat=null;try{stat=fs.statSync(gamedirpath);}
catch(ex){};if(!stat||!stat.isDirectory()){try{fs.mkdirSync(path_mod.join(userpath,'games'));}
catch(ex){};try{fs.mkdirSync(gamedirpath);}
catch(ex){};stat=null;try{stat=fs.statSync(gamedirpath);}
catch(ex){};if(!stat||!stat.isDirectory()){GlkOte.log('Unable to create gamedirpath: '+gamedirpath);return;}}
var pathj=path_mod.join(gamedirpath,'autosave.json');var pathr=path_mod.join(gamedirpath,'autosave.ram');if(!snapshot){try{fs.unlinkSync(pathj);}
catch(ex){};try{fs.unlinkSync(pathr);}
catch(ex){};return;}
var ram=snapshot.ram;snapshot.ram=undefined;var str=JSON.stringify(snapshot);fs.writeFileSync(pathj,str,{encoding:'utf8'});if(ram){var buf=buffer_mod.Buffer.from(ram);fs.writeFileSync(pathr,buf);}}
function autosave_read(signature)
{var gamedirpath=path_mod.join(userpath,'games',signature);var pathj=path_mod.join(gamedirpath,'autosave.json');var pathr=path_mod.join(gamedirpath,'autosave.ram');try{var str=fs.readFileSync(pathj,{encoding:'utf8'});var snapshot=JSON.parse(str);try{var buf=fs.readFileSync(pathr,{encoding:null});var ram=Array.from(buf.values());snapshot.ram=ram;}
catch(ex){};return snapshot;}
...
return{classname:'Dialog (ElectroFS)',streaming:true,init_async:init_async,inited:dialog_inited,getlibrary:dialog_get_library,open:dialog_open,file_clean_fixed_name:file_clean_fixed_name,file_construct_ref:file_construct_ref,file_construct_temp_ref:file_construct_temp_ref,file_ref_exists:file_ref_exists,file_remove_ref:file_remove_ref,file_fopen:file_fopen,file_write:file_write,file_read:file_read,autosave_write:autosave_write,autosave_read:autosave_read};};var Dialog=new DialogClass();try{exports.Dialog=Dialog;exports.DialogClass=DialogClass;}catch(ex){};'use strict';var BlorbClass=function(){var inited=false;var metadata={};var coverimageres=undefined;var debug_info=null;var blorbchunks={};function blorb_init(data,opts){if(inited){throw new Error('Blorb: already inited');}
...
return null;return{data:chunk.content,type:chunk.type,binary:chunk.binary};}
