...
typeclass='GraphicsWindow';var rockclass='WindowRock_'+arg.rock;frameel=$('<div>',{id:'window'+arg.id,'class':'WindowFrame HasNoInputField '+typeclass+' '+rockclass});frameel.data('winid',arg.id);frameel.on('mousedown',arg.id,evhan_window_mousedown);if(perform_paging&&win.type=='buffer')
...
var divel=buffer_last_line(win);if(divel){var cursel=$('<span>',{id:'win'+win.id+'_cursor','class':'InvisibleCursor'});divel.append(cursel);if(win.inputel){var inputel=win.inputel;var pos=cursel.position();var width=win.frameel.width()-(current_metrics.buffermarginx+pos.left+2);if(width<1)
...
if(win.type=='buffer'){var cursel=$('#win'+win.id+'_cursor',dom_context);if(!cursel.length){cursel=$('<span>',{id:'win'+win.id+'_cursor','class':'InvisibleCursor'});win.frameel.append(cursel);}
...
return{version:'2.2.5',init:glkote_init,update:glkote_update,extevent:glkote_extevent,getinterface:glkote_get_interface,getdomcontext:glkote_get_dom_context,setdomcontext:glkote_set_dom_context,save_allstate:glkote_save_allstate,log:glkote_log,warning:glkote_warning,error:glkote_error};}();try{exports.GlkOte=GlkOte;}catch(ex){};var Dialog=function(){const electron=require('electron');const fs=require('fs');const path_mod=require('path');const buffer_mod=require('buffer');var inited=false;var userpath=null;var temppath=null;var extfilepath=null;const filemode_Write=0x01;const filemode_Read=0x02;const filemode_ReadWrite=0x03;const filemode_WriteAppend=0x05;const seekmode_Start=0;const seekmode_Current=1;const seekmode_End=2;const fileusage_Data=0x00;const fileusage_SavedGame=0x01;const fileusage_Transcript=0x02;const fileusage_InputRecord=0x03;const BUFFER_SIZE=256;function init_async(callback)
