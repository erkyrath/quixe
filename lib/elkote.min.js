...
return got;}
if(this.bufuse)
this.fflush();this.bufuse=filemode_Read;this.bufmark=0;this.buflen=fs.readSync(this.fd,this.buffer,0,BUFFER_SIZE,this.mark);if(this.buflen==0){this.bufuse=0;return got;}
this.mark+=this.buflen;}},fwrite:function(buf,len){if(len===undefined)
...
this.fflush();this.bufuse=filemode_Write;this.buflen=0;}},ftell:function(){if(this.bufuse==filemode_Read){return this.mark-(this.buflen-this.bufmark);}
...
else if(seekmode==seekmode_End){try{var stats=fs.fstatSync(fstream.fd);val=stats.size+pos;}
catch(ex){val=this.mark+pos;}}
else{val=pos;}
if(val<0)
val=0;this.mark=val;},fflush:function(){if(this.bufuse==filemode_Read){}
...
else{if(!str.isbinary){var len=str.buffer4.write(String.fromCharCode(ch));str.fstream.fwrite(str.buffer4,len);}
else{str.buffer4[0]=0;str.buffer4[1]=0;str.buffer4[2]=0;str.buffer4[3]=ch;str.fstream.fwrite(str.buffer4,4);}}
break;}
gli_stream_dirty_file(str);case strtype_Memory:if(str.bufpos<str.buflen){str.buf[str.bufpos]=ch;str.bufpos+=1;if(str.bufpos>str.bufeof)
...
else{if(!str.isbinary){if(allbytes){var buf=new str.fstream.BufferClass(arr);str.fstream.fwrite(buf);}
else{var arr8=UniArrayToUTF8(arr);var buf=new str.fstream.BufferClass(arr8);str.fstream.fwrite(buf);}}
else{var buf=new str.fstream.BufferClass(4*arr.count);for(ix=0;ix<arr.length;ix++){buf.writeUInt32BE(arr[ix],4*ix,true);}
str.fstream.fwrite(buf);}}
break;}
gli_stream_dirty_file(str);case strtype_Memory:len=arr.length;if(len>str.buflen-str.bufpos)
...
return 63;return ch;}
case strtype_File:if(str.streaming){if(!str.unicode){var len=str.fstream.fread(str.buffer4,1);if(!len)
...
return lx;}
case strtype_File:if(str.streaming){if(len==0)
return 0;len-=1;gotnewline=false;for(lx=0;lx<len&&!gotnewline;lx++){ch=gli_get_char(str,want_unicode);if(ch==-1)
break;buf[lx]=ch;gotnewline=(ch==10);}
return lx;}
...
break;buf[lx]=ch;}
return lx;}
case strtype_File:if(str.streaming){for(lx=0;lx<len;lx++){ch=gli_get_char(str,want_unicode);if(ch==-1)
break;buf[lx]=ch;}
return lx;}
...
throw('glk_put_jstring: invalid stream');str.writecount+=val.length;switch(str.type){case strtype_File:if(str.streaming){if(!str.unicode){if(allbytes){var buf=new str.fstream.BufferClass(val,'binary');str.fstream.fwrite(buf);}
else{var buf=new str.fstream.BufferClass(val);str.fstream.fwrite(buf);}}
...
str.fstream.fwrite(buf);}}
break;}
gli_stream_dirty_file(str);case strtype_Memory:len=val.length;if(len>str.buflen-str.bufpos)
...
else{fstream=Dialog.file_fopen(fmode,fref.ref);}
str=gli_new_stream(strtype_File,(fmode!=Const.filemode_Write),(fmode!=Const.filemode_Read),rock);str.unicode=false;str.ref=fref.ref;if(!Dialog.streaming){str.streaming=false;str.buf=content;str.buflen=0xFFFFFFFF;if(fmode==Const.filemode_Write)
str.bufeof=0;else
...
return null;var buf=el.data;var isbinary=(el.type=='BINA');str=gli_new_stream(strtype_Resource,true,false,rock);str.unicode=false;str.isbinary=isbinary;str.streaming=false;if(buf){str.buf=buf;str.buflen=buf.length;str.bufpos=0;str.bufeof=str.buflen;}
return str;}
...
function glk_fileref_create_by_name(usage,filename,rock){filename=Dialog.file_clean_fixed_name(filename,usage);fref=gli_new_fileref(filename,usage,rock,null);return fref;}
...
str=gli_new_stream(strtype_File,(fmode!=Const.filemode_Write),(fmode!=Const.filemode_Read),rock);str.unicode=true;str.ref=fref.ref;if(!Dialog.streaming){str.streaming=false;str.buf=content;str.buflen=0xFFFFFFFF;if(fmode==Const.filemode_Write)
str.bufeof=0;else
str.bufeof=content.length;if(fmode==Const.filemode_WriteAppend)
str.bufpos=str.bufeof;else
str.bufpos=0;}
