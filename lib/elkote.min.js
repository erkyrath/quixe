...
return{version:'2.2.0',init:glkote_init,update:glkote_update,extevent:glkote_extevent,getinterface:glkote_get_interface,getdomcontext:glkote_get_dom_context,setdomcontext:glkote_set_dom_context,log:glkote_log,warning:glkote_warning,error:glkote_error};}();Dialog=function(){const fs=require('fs');const path_mod=require('path');const buffer_mod=require('buffer');var userpath=require('electron').remote.app.getPath('userData');var extfilepath=path_mod.join(userpath,'quixe-files');try{fs.mkdirSync(extfilepath);}
catch(ex){}
...
return{streaming:true,open:dialog_open,file_clean_fixed_name:file_clean_fixed_name,file_construct_ref:file_construct_ref,file_ref_exists:file_ref_exists,file_remove_ref:file_remove_ref,file_fopen:file_fopen,file_write:file_write,file_read:file_read};}();Glk=function(){var VM=null;var has_canvas;var option_exit_warning;var has_exited=false;var ui_disabled=false;var ui_specialinput=null;var ui_specialcallback=null;var event_generation=0;var current_partial_inputs=null;var current_partial_outputs=null;function init(vm_options){has_canvas=(document.createElement('canvas').getContext!=undefined);VM=vm_options.vm;if(window.GiDispa)
GiDispa.set_vm(VM);vm_options.accept=accept_ui_event;GlkOte.init(vm_options);option_exit_warning=vm_options.exit_warning;}
...
str=gli_new_stream(strtype_File,(fmode!=Const.filemode_Write),(fmode!=Const.filemode_Read),rock);str.unicode=false;str.isbinary=!fref.textmode;str.ref=fref.ref;if(!Dialog.streaming){str.streaming=false;str.buf=content;str.buflen=0xFFFFFFFF;if(fmode==Const.filemode_Write)
str.bufeof=0;else
...
str=gli_new_stream(strtype_File,(fmode!=Const.filemode_Write),(fmode!=Const.filemode_Read),rock);str.unicode=true;str.isbinary=!fref.textmode;str.ref=fref.ref;if(!Dialog.streaming){str.streaming=false;str.buf=content;str.buflen=0xFFFFFFFF;if(fmode==Const.filemode_Write)
str.bufeof=0;else
str.bufeof=content.length;if(fmode==Const.filemode_WriteAppend)
str.bufpos=str.bufeof;else
str.bufpos=0;}
