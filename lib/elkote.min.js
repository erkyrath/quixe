...
debugmod.open();}}
if(iface.GiLoad){GiLoad=iface.GiLoad;}
...
if(autorestore){if(autorestore.history){jQuery.each(autorestore.history,function(winid,ls){var win=windowdic[winid];if(win!=null){win.history=ls.slice(0);win.historypos=win.history.length;}});}
if(autorestore.defcolor){jQuery.each(autorestore.defcolor,function(winid,val){var win=windowdic[winid];if(win!=null){win.defcolor=val;}});}
...
function close_one_window(win){win.frameel.remove();delete windowdic[win.id];win.frameel=null;var moreel=$('#'+dom_prefix+'win'+win.id+'_moreprompt',dom_context);if(moreel.length)
moreel.remove();}
...
for(sx=0;sx<content.length;sx++){var rdesc=content[sx];var rstyle,rtext,rlink;if(jQuery.type(rdesc)==='object'){if(rdesc.special!==undefined){if(rdesc.special=='image'){var imgurl=rdesc.url;if(GiLoad&&GiLoad.get_image_url){var newurl=GiLoad.get_image_url(rdesc.image);if(newurl)
imgurl=newurl;}
...
hasinput[argi.id]=argi;});jQuery.each(windowdic,function(winid,win){if(win.input){var argi=hasinput[win.id];if(argi==null||argi.gen>win.input.gen){win.input=null;win.frameel.addClass('HasNoInputField');win.frameel.removeClass('HasInputField');if(win.inputel){win.inputel.remove();win.inputel=null;}}}});}
...
return;win.input=argi;win.frameel.addClass('HasInputField');win.frameel.removeClass('HasNoInputField');var maxlen=1;if(argi.type=='line')
...
function glkote_get_library(val){switch(val){case'Dialog':return Dialog;case'GiLoad':return GiLoad;}
return null;}
...
if(!loadedimg){var imgurl=op.url;if(GiLoad&&GiLoad.get_image_url){var newurl=GiLoad.get_image_url(op.image);if(newurl)
imgurl=newurl;}
...
if(!(type=='init'||type=='refresh'||type=='specialresponse'||type=='debuginput')){jQuery.each(windowdic,function(tmpid,win){var savepartial=(type!='line'&&type!='char')||(win.id!=winid);if(savepartial&&win.input&&win.input.type=='line'&&win.inputel&&win.inputel.val()){var partial=res.partial;if(!partial){partial={};res.partial=partial;};partial[win.id]=win.inputel.val();}});}
...
function evhan_doc_pixelreschange(ev){var ratio=window.devicePixelRatio||1;if(ratio!=current_devpixelratio){current_devpixelratio=ratio;jQuery.each(windowdic,function(winid,win){if(win.type=='graphics'){var el=$('#'+dom_prefix+'win'+win.id+'_canvas',dom_context);win.scaleratio=current_devpixelratio/win.backpixelratio;var ctx=canvas_get_2dcontext(el);el.attr('width',win.graphwidth*win.scaleratio);el.attr('height',win.graphheight*win.scaleratio);el.css('width',(win.graphwidth+'px'));el.css('height',(win.graphheight+'px'));if(ctx){ctx.setTransform(win.scaleratio,0,0,win.scaleratio,0,0);ctx.fillStyle=win.defcolor;ctx.fillRect(0,0,win.graphwidth,win.graphheight);ctx.fillStyle='#000000';}
...
event_generation+=1;current_partial_inputs=obj.partial;switch(obj.type){case'init':content_metrics=complete_metrics(obj.metrics);VM.init();break;case'external':var res=null;if(option_extevent_hook){res=option_extevent_hook(obj.value);}
...
function get_library(val){switch(val){case'VM':return VM;case'GlkOte':return GlkOte;case'GiDispa':return GiDispa;case'GiLoad':return GiLoad;case'Dialog':return GlkOte.getlibrary('Dialog');}
return null;}
...
return str;}
function glk_stream_open_resource(filenum,rock){var str;if(!GiLoad||!GiLoad.find_data_chunk)
...
return str;}
function glk_stream_open_resource_uni(filenum,rock){var str;if(!GiLoad||!GiLoad.find_data_chunk)
return null;var el=GiLoad.find_data_chunk(filenum);if(!el)
...
function glk_image_get_info(imgid,widthref,heightref){if(!GiLoad||!GiLoad.get_image_info)
...
throw('glk_image_draw: invalid window');if(!GiLoad||!GiLoad.get_image_info)
...
throw('glk_image_draw_scaled: invalid window');if(!GiLoad||!GiLoad.get_image_info)
return 0;var info=GiLoad.get_image_info(imgid);if(!info)
