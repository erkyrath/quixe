...
if(divel==null){divel=$('<div>',{'class':'BufferLine'});divel.data('blankpara',true);win.frameel.append(divel);}
...
{const dialog=require('electron').remote.dialog;var opts={filters:filters_for_usage(usage)};var mainwin=require('electron').remote.getCurrentWindow();if(!tosave){opts.properties=['openFile'];dialog.showOpenDialog(mainwin,opts).then(function(res){var ls=res.filePaths;if(res.canceled||!ls||!ls.length){callback(null);}
