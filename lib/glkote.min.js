...
return elements;};return function(className,parentElement){return $(parentElement||document.body).getElementsByClassName(className);};}(Element.Methods);Element.ClassNames=Class.create();Element.ClassNames.prototype={initialize:function(element){this.element=$(element);},_each:function(iterator){this.element.className.split(/\s+/).select(function(name){return name.length>0;})._each(iterator);},set:function(className){this.element.className=className;},add:function(classNameToAdd){if(this.include(classNameToAdd))return;this.set($A(this).concat(classNameToAdd).join(' '));},remove:function(classNameToRemove){if(!this.include(classNameToRemove))return;this.set($A(this).without(classNameToRemove).join(' '));},toString:function(){return $A(this).join(' ');}};Object.extend(Element.ClassNames.prototype,Enumerable);function AudioFileRequest(url,async){this.url=url;if(typeof async=='undefined'||async==null){async=true;}
this.async=async;var splitURL=url.split('.');this.extension=splitURL[splitURL.length-1].toLowerCase();}
AudioFileRequest.prototype.onSuccess=function(decoded){};AudioFileRequest.prototype.onFailure=function(decoded){};AudioFileRequest.prototype.send=function(){if(this.extension!='wav'&&this.extension!='aiff'&&this.extension!='aif'){this.onFailure();return;}
var request=new XMLHttpRequest();request.open('GET',this.url,this.async);request.overrideMimeType('text/plain; charset=x-user-defined');request.onreadystatechange=function(event){if(request.readyState==4){if(request.status==200||request.status==0){this.handleResponse(request.responseText);}
else{this.onFailure();}}}.bind(this);request.send(null);};AudioFileRequest.prototype.handleResponse=function(data){var decoder,decoded;if(this.extension=='wav'){decoder=new WAVDecoder();decoded=decoder.decode(data);}
else if(this.extension=='aiff'||this.extension=='aif'){decoder=new AIFFDecoder();decoded=decoder.decode(data);}
this.onSuccess(decoded);};function Decoder(){}
Decoder.prototype.readString=function(data,offset,length){return data.slice(offset,offset+length);};Decoder.prototype.readIntL=function(data,offset,length){var value=0;for(var i=0;i<length;i++){value=value+((data.charCodeAt(offset+i)&0xFF)*Math.pow(2,8*i));}
return value;};Decoder.prototype.readChunkHeaderL=function(data,offset){var chunk={};chunk.name=this.readString(data,offset,4);chunk.length=this.readIntL(data,offset+4,4);return chunk;};Decoder.prototype.readIntB=function(data,offset,length){var value=0;for(var i=0;i<length;i++){value=value+((data.charCodeAt(offset+i)&0xFF)*Math.pow(2,8*(length-i-1)));}
return value;};Decoder.prototype.readChunkHeaderB=function(data,offset){var chunk={};chunk.name=this.readString(data,offset,4);chunk.length=this.readIntB(data,offset+4,4);return chunk;};Decoder.prototype.readFloatB=function(data,offset){var expon=this.readIntB(data,offset,2);var range=1<<16-1;if(expon>=range){expon|=~(range-1);}
var sign=1;if(expon<0){sign=-1;expon+=range;}
var himant=this.readIntB(data,offset+2,4);var lomant=this.readIntB(data,offset+6,4);var value;if(expon==himant==lomant==0){value=0;}
else if(expon==0x7FFF){value=Number.MAX_VALUE;}
else{expon-=16383;value=(himant*0x100000000+lomant)*Math.pow(2,expon-63);}
return sign*value;};function WAVDecoder(data){}
WAVDecoder.prototype.__proto__=Decoder.prototype;WAVDecoder.prototype.decode=function(data){var decoded={};var offset=0;var chunk=this.readChunkHeaderL(data,offset);offset+=8;if(chunk.name!='RIFF'){console.error('File is not a WAV');return null;}
var fileLength=chunk.length;fileLength+=8;var wave=this.readString(data,offset,4);offset+=4;if(wave!='WAVE'){console.error('File is not a WAV');return null;}
while(offset<fileLength){var chunk=this.readChunkHeaderL(data,offset);offset+=8;if(chunk.name=='fmt '){var encoding=this.readIntL(data,offset,2);offset+=2;if(encoding!=0x0001){console.error('Cannot decode non-PCM encoded WAV file');return null;}
var numberOfChannels=this.readIntL(data,offset,2);offset+=2;var sampleRate=this.readIntL(data,offset,4);offset+=4;offset+=4;offset+=2;var bitDepth=this.readIntL(data,offset,2);var bytesPerSample=bitDepth/8;offset+=2;}
else if(chunk.name=='data'){var length=chunk.length/(bytesPerSample*numberOfChannels);var channels=[];for(var i=0;i<numberOfChannels;i++){channels.push(new Float32Array(length));}
for(var i=0;i<numberOfChannels;i++){var channel=channels[i];for(var j=0;j<length;j++){var index=offset;index+=(j*numberOfChannels+i)*bytesPerSample;var value=this.readIntL(data,index,bytesPerSample);var range=1<<bitDepth-1;if(value>=range){value|=~(range-1);}
channel[j]=value/range;}}
offset+=chunk.length;}
else{offset+=chunk.length;}}
decoded.sampleRate=sampleRate;decoded.bitDepth=bitDepth;decoded.channels=channels;decoded.length=length;return decoded;};function AIFFDecoder(){}
AIFFDecoder.prototype.__proto__=Decoder.prototype;AIFFDecoder.prototype.decode=function(data){var decoded={};var offset=0;var chunk=this.readChunkHeaderB(data,offset);offset+=8;if(chunk.name!='FORM'){console.error('File is not an AIFF');return null;}
var fileLength=chunk.length;fileLength+=8;var aiff=this.readString(data,offset,4);offset+=4;if(aiff!='AIFF'){console.error('File is not an AIFF');return null;}
while(offset<fileLength){var chunk=this.readChunkHeaderB(data,offset);offset+=8;if(chunk.name=='COMM'){var numberOfChannels=this.readIntB(data,offset,2);offset+=2;var length=this.readIntB(data,offset,4);offset+=4;var channels=[];for(var i=0;i<numberOfChannels;i++){channels.push(new Float32Array(length));}
var bitDepth=this.readIntB(data,offset,2);var bytesPerSample=bitDepth/8;offset+=2;var sampleRate=this.readFloatB(data,offset);offset+=10;}
else if(chunk.name=='SSND'){var dataOffset=this.readIntB(data,offset,4);offset+=4;offset+=4;offset+=dataOffset;for(var i=0;i<numberOfChannels;i++){var channel=channels[i];for(var j=0;j<length;j++){var index=offset;index+=(j*numberOfChannels+i)*bytesPerSample;var value=this.readIntB(data,index,bytesPerSample);var range=1<<bitDepth-1;if(value>=range){value|=~(range-1);}
channel[j]=value/range;}}
offset+=chunk.length-dataOffset-8;}
else{offset+=chunk.length;}}
decoded.sampleRate=sampleRate;decoded.bitDepth=bitDepth;decoded.channels=channels;decoded.length=length;return decoded;};GlkOte=function(){var game_interface=null;var windowport_id='windowport';var gameport_id='gameport';var generation=0;var disabled=false;var loading_visible=null;var windowdic=null;var current_metrics=null;var currently_focussed=false;var last_known_focus=0;var last_known_paging=0;var windows_paging_count=0;var resize_timer=null;var retry_timer=null;var is_ie7=false;var perform_paging=true;var detect_external_links=false;var regex_external_links=null;var capabilities=null;var NBSP="\xa0";var max_buffer_length=200;var terminator_key_names={escape:Event.KEY_ESC,func1:112,func2:113,func3:114,func4:115,func5:116,func6:117,func7:118,func8:119,func9:120,func10:121,func11:122,func12:123};var terminator_key_values={};function glkote_init(iface){if(!iface&&window.Game)
...
capabilities={};var extra={capabilities:capabilities};capabilities.canvas=!!window.CanvasRenderingContext2D;capabilities.audio=!!AudioContext;send_response('init',null,current_metrics,extra);}
...
accept_specialinput(arg.specialinput);if(arg.sound!=null)
accept_sound(arg.sound);windowdic.values().each(function(win){if(win.type=='buffer'&&win.needscroll){win.needscroll=false;if(!win.needspaging){var frameel=win.frameel;if(!perform_paging){frameel.scrollTop=frameel.scrollHeight;win.needspaging=false;}
...
typeclass='GraphicsWindow';var rockclass='WindowRock_'+arg.rock;frameel=new Element('div',{id:'window'+arg.id,'class':'WindowFrame '+typeclass+' '+rockclass});frameel.winid=arg.id;Event.observe(frameel,'mousedown',function(ev){evhan_window_mousedown(ev,frameel);});if(perform_paging&&win.type=='buffer')
...
if(win.type=='buffer'){}
if(win.type=='graphics'&&capabilities.canvas){win.frameel.backgroundColor=arg.bgcolor;var canvas=win.canvas;if(!canvas||canvas.width!==arg.width||canvas.height!==arg.height){var newcanvas=win.canvas=new Element('canvas');newcanvas.width=arg.width;newcanvas.height=arg.height;var ctx=newcanvas.getContext("2d");ctx.fillStyle=arg.bgcolor;ctx.fillRect(0,0,newcanvas.width,newcanvas.height);if(canvas){ctx.drawImage(canvas,0,0);}
...
for(sx=0;sx<content.length;sx++){var rdesc=content[sx];if(rdesc.image){var el=rdesc.image.cloneNode();if(rdesc.width){el.style.width=rdesc.width+"px";el.style.height=rdesc.height+"px";}
if(rdesc.floated){el.addClassName("image-floated-"+rdesc.floated);}
if(rdesc.align){el.addClassName("image-inline-"+rdesc.align);}
divel.insert(el);continue;}
if(rdesc.clear){var el=new Element('span',{'class':'float-clear'});divel.insert(el);continue;}
var rstyle,rtext,rlink;if(rdesc.length===undefined){rstyle=rdesc.style;rtext=rdesc.text;rlink=rdesc.hyperlink;}
else{rstyle=rdesc;sx++;rtext=content[sx];rlink=undefined;}
var el=new Element('span');if(rstyle==='blockquote'){divel.addClassName('Style_'+rstyle);}
else{el.addClassName('Style_'+rstyle);}
rtext=rtext.replace(regex_long_whitespace,func_long_whitespace);if(divel.endswhite){rtext=rtext.replace(regex_initial_whitespace,NBSP);}
...
if(win.type=='graphics'&&capabilities.canvas){win.needscroll=false;var ctx=win.canvas.getContext("2d");ctx.fillStyle=arg.bgcolor;win.frameel.style.backgroundColor=arg.bgcolor;for(var i=0,l=arg.drawstack.length;i<l;i++){var drawcmd=arg.drawstack[i];if(drawcmd.image){if(drawcmd.width){ctx.drawImage(drawcmd.image,drawcmd.x,drawcmd.y,draw_cmd.width,draw_cmd.height);}
else{ctx.drawImage(drawcmd.image,drawcmd.x,drawcmd.y);}}
else if(drawcmd.color){ctx.fillStyle=drawcmd.color;ctx.fillRect(drawcmd.x,drawcmd.y,drawcmd.width,drawcmd.height);}
else if(drawcmd.clear){ctx.fillStyle=arg.bgcolor;ctx.fillRect(0,0,win.canvas.width,win.canvas.height);}}}}
...
var AudioContext=window.AudioContext||window.webkitAudioContext;var audio_context;if(AudioContext){audio_context=new AudioContext();}
var audio_channels={};function AudioChannel(identifier){this.identifier=identifier;audio_channels[identifier]=this;this.gain_node=audio_context.createGain();this.gain_node.connect(audio_context.destination);this.set_volume(1.0);this.source=null;this.start_time=null;this.stop_timer=null;this.stop_total_time=null;this.paused=false;this.resume_track=null;this.resume_offset=0;this.resume_loop=null;}
AudioChannel.prototype.play=function(track,times){this.stop();var offset=0;var loop;if(times<0){loop=true;}
else if(times===0){return;}
else if(times===1){loop=false;}
else{loop=times;}
if(this.paused){this.resume_track=track;this.resume_offset=offset;this.resume_loop=loop;}
else{this._play(track,offset,loop);}};AudioChannel.prototype._play=function(track,offset,loop){this.stop();this.resume_track=null;this.resume_offset=0;this.resume_loop=null;this.source=audio_context.createBufferSource();this.source.connect(this.gain_node);this.source.buffer=track;if(loop!==false){this.source.loop=true;if(loop!==true){this._stop_in(track.duration*loop);}}
this.start_time=audio_context.currentTime-offset;this.source.start(0,offset%track.duration);};AudioChannel.prototype._clear_stop=function(){if(!this.stop_timer)return;window.clearTimeout(this.stop_timer);this.stop_timer=null;};AudioChannel.prototype._stop_in=function(time){this._clear_stop();this.stop_timer=window.setTimeout(this.stop.bind(this),time*1000);this.stop_total_time=time;};AudioChannel.prototype.set_volume=function(volume,duration){this.gain_node.gain.cancelScheduledValues(audio_context.currenTime);this.gain_node.gain.linearRampToValueAtTime(volume,duration||0);};AudioChannel.prototype.stop=function(){this._clear_stop();if(this.source){this.source.stop();this.source=null;}};AudioChannel.prototype.pause=function(){if(this.paused)return;this.paused=true;if(this.source){var time_since_start=audio_context.currentTime-this.start_time;this.resume_track=this.source.buffer;this.resume_offset=time_since_start;if(this.stop_timer){this.resume_loop=this.stop_total_time-time_since_start;}
else{this.resume_loop=this.source.loop;}}
else{this.resume_track=null;this.resume_offset=0;this.resume_loop=null;}
this.stop();};AudioChannel.prototype.unpause=function(){if(!this.paused)return;this.paused=false;if(this.resume_track){this._play(this.resume_track,this.resume_offset,this.resume_loop);}};function accept_sound(sound_commands){for(var i=0,l=sound_commands.length;i<l;i++){var channel_id=sound_commands[i][0];var method=sound_commands[i][1];var args=sound_commands[i].slice(2);if(method==='create'){new AudioChannel(args[0]);}
else{var channel=audio_channels[channel_id];channel[method].apply(channel,args);}}}
...
else if(type=='init'||type=='arrange'){res.metrics=val;res.extra=val2;}
...
Event.observe(window,'storage',evhan_storage_changed);return{open:dialog_open,file_construct_ref:file_construct_ref,file_ref_exists:file_ref_exists,file_remove_ref:file_remove_ref,file_write:file_write,file_read:file_read};}();Glk=function(){var VM=null;var has_exited=false;var ui_disabled=false;var ui_specialinput=null;var ui_specialcallback=null;var event_generation=0;var current_partial_inputs=null;var current_partial_outputs=null;var client_capabilities=null;function init(vm_options){VM=vm_options.vm;if(window.GiDispa)
...
event_generation+=1;current_partial_inputs=obj.partial;switch(obj.type){case'init':content_metrics=obj.metrics;if(obj.extra)
client_capabilities=obj.extra.capabilities;else
client_capabilities={};VM.init();break;case'external':if(obj.value=='timer'){handle_timer_input();}
...
continue;obj={id:win.disprock,rock:win.rock};winarray.push(obj);switch(win.type){case Const.wintype_TextBuffer:obj.type='buffer';break;case Const.wintype_TextGrid:obj.type='grid';obj.gridwidth=win.gridwidth;obj.gridheight=win.gridheight;break;case Const.wintype_Graphics:obj.type='graphics';break;}
...
useobj=obj.lines.length;break;case Const.wintype_Graphics:obj.bgcolor=decode_color(win.bgcolor);obj.drawstack=win.drawstack;useobj=obj.drawstack.length;win.drawstack=[];break;}
if(useobj)
...
if(obj)
inputarray.push(obj);}
var soundarray=[];for(var i=0,l=gli_schannellist.length;i<l;i++){var schan=gli_schannellist[i];if(schan.destroyed)
continue;soundarray=soundarray.concat(schan.pending_updates);schan.pending_updates=[];}
dataobj.windows=winarray;dataobj.content=contentarray;dataobj.input=inputarray;dataobj.sound=soundarray;if(ui_specialinput){dataobj.specialinput=ui_specialinput;}
...
else if(win.pair_division==Const.winmethod_Fixed){split=0;if(!win.pair_key){}
else if(win.pair_key.type==Const.wintype_TextBuffer){if(!win.pair_vertical)
...
else if(win.pair_key.type==Const.wintype_TextGrid){if(!win.pair_vertical)
...
else if(win.pair_key.type==Const.wintype_Graphics){split=win.pair_size;}
else{throw('gli_window_rearrange: unsupported window type for fixed sizing');}
...
arr[0]=1;return 0;}
if(arr)
arr[0]=1;return 2;case 4:return 0;case 5:return 1;case 6:return 1;case 7:if(val===Const.wintype_Graphics)
return client_capabilities.canvas?1:0;else if(val===Const.wintype_TextBuffer)
return 1;else
return 0;case 8:return client_capabilities.audio?1:0;case 9:return client_capabilities.audio?1:0;case 10:return 0;case 11:return 1;case 12:if(val==3||val==4)
return 1;else
...
newwin=gli_new_window(wintype,rock);switch(newwin.type){case Const.wintype_TextBuffer:newwin.accum=[];newwin.accumstyle=null;newwin.accumhyperlink=0;newwin.content=[];newwin.clearcontent=false;break;case Const.wintype_TextGrid:newwin.gridwidth=0;newwin.gridheight=0;newwin.lines=[];newwin.cursorx=0;newwin.cursory=0;break;case Const.wintype_Graphics:newwin.bgcolor=0xffffff;newwin.drawstack=[];break;case Const.wintype_Blank:break;case Const.wintype_Pair:throw('glk_window_open: cannot open pair window directly')
...
break;case Const.wintype_Graphics:win.drawstack=[{clear:true}];break;}}
...
function glk_image_get_info(imgid,widthref,heightref){var chunk=GiLoad.find_picture_chunk(imgid);if(!chunk)
return 0;if(widthref)
widthref.set_value(chunk.width);if(heightref)
heightref.set_value(chunk.height);return 1;}
function glk_image_draw(win,imgid,val1,val2){return glk_image_draw_scaled(win,imgid,val1,val2,null,null);}
function _append_to_window_content(win,item){if(!win.content||!win.content.length){win.content=[{}];}
var last_row=win.content[win.content.length-1];if(!last_row.content){last_row.content=[];}
last_row.content.push(item);}
function glk_image_draw_scaled(win,imgid,val1,val2,width,height){if(!win)
throw('glk_image_draw: invalid window');if(win.type==Const.wintype_Graphics&&!client_capabilities.canvas)
return 0;var chunk=GiLoad.find_picture_chunk(imgid);if(!chunk)
return 0;if(win.type==Const.wintype_Graphics){win.drawstack.push({image:chunk.image,x:val1,y:val2,width:width,height:height});}
else if(win.type==Const.wintype_TextBuffer){gli_window_buffer_deaccumulate(win);var special_token={image:chunk.image,width:width,height:height};if(val1==Const.imagealign_InlineUp){special_token.align="top";}
else if(val1==Const.imagealign_InlineDown){special_token.align="bottom";}
else if(val1==Const.imagealign_InlineCenter){special_token.align="middle";}
else if(val1==Const.imagealign_MarginLeft){special_token.floated="left";}
else if(val1==Const.imagealign_MarginRight){special_token.floated="right";}
_append_to_window_content(win,special_token);}
else{throw('glk_image_draw: invalid window type');}
return 1;}
function glk_window_flow_break(win){if(!win)
throw('glk_window_flow_break: invalid window');if(win.type==Const.wintype_TextBuffer){gli_window_buffer_deaccumulate(win);_append_to_window_content(win,{clear:true});}}
function glk_window_erase_rect(win,left,top,width,height){glk_window_fill_rect(win,win.bgcolor,left,top,width,height);}
function decode_color(color){var a=(color&0xff000000)>>24;if(a)
throw('invalid color');return"#"+("000000"+color.toString(16)).slice(-6);}
...
throw('glk_window_fill_rect: invalid window type');win.drawstack.push({color:decode_color(color),x:left,y:top,width:width,height:height});}
...
throw('glk_window_fill_rect: invalid window type');win.bgcolor=color;}
function SoundChannel(rock){this.rock=rock;this.index=gli_schannellist.length;this.destroyed=false;this.pending_updates=[];gli_schannellist.push(this);if(window.GiDispa)
GiDispa.class_register('schannel',this);else
this.disprock=gli_api_display_rocks++;this.queue(['create',this.disprock]);}
SoundChannel.prototype.destroy=function(){this.queue(['stop']);this.queue(['destroy']);this.destroyed=true;};SoundChannel.prototype.queue=function(method_and_arguments){if(this.destroyed)
throw('invalid schannel');this.pending_updates.push([this.disprock].concat(method_and_arguments));};function glk_schannel_iterate(schan,rockref){var index=schan?schan.index+1:0;while(index<gli_schannellist.length&&gli_schannellist[index].destroyed){index++;}
schan=gli_schannellist[index];if(!schan||schan.destroyed)
return null;if(schan){if(rockref)
rockref.set_value(schan.rock);return schan;}
if(rockref)
rockref.set_value(0);return null;}
function glk_schannel_get_rock(schan){if(!schan||schan.destroyed)
throw('glk_schannel_get_rock: invalid schannel');return schan.rock;}
function glk_schannel_create(rock){return glk_schannel_create_ext(rock,0x10000);}
function glk_schannel_create_ext(rock,vol){var schan=new SoundChannel(rock);schan.queue(['set_volume',vol/0x10000]);return schan;}
function glk_schannel_destroy(schan){if(!schan||schan.destroyed)
throw('glk_schannel_destroy: invalid schannel');schan.destroy();return null;}
function glk_schannel_play(schan,sndid){return glk_schannel_play_ext(schan,sndid,1,0);}
function glk_schannel_play_ext(schan,sndid,repeats,notify){if(!schan||schan.destroyed)
throw('glk_schannel_play_ext: invalid schannel');var chunk=GiLoad.find_sound_chunk(sndid);if(!chunk)
return 0;if(repeats==0xffffffff){repeats=-1;}
schan.queue(['play',chunk.track,repeats]);return 1;}
function glk_schannel_stop(schan){if(!schan||schan.destroyed)
throw('glk_schannel_stop: invalid schannel');schan.queue(['stop']);}
function glk_schannel_set_volume(schan,vol){if(!schan||schan.destroyed)
throw('glk_schannel_pause: invalid schannel');schan.queue(['set_volume',vol/0x10000]);}
function glk_sound_load_hint(sndid,flag){return null;}
function glk_schannel_play_multi(schans,sndids,notify){var retval=0;for(var i=0,l=schans.length;i<l;i++){var schan=schans[i];var sndid=sndids[i];if(!schan||schan.destroyed)
throw('glk_schannel_play_multi: invalid schannel');var chunk=GiLoad.find_sound_chunk(sndid);if(!chunk)
continue;schan.queue(['play',chunk.track,1]);retval+=1;}
return retval;}
function glk_schannel_pause(schan){if(!schan||!gli_schannellist[schan.index])
throw('glk_schannel_pause: invalid schannel');schan.queue(['pause']);return null;}
function glk_schannel_unpause(schan){if(!schan||!gli_schannellist[schan.index])
throw('glk_schannel_unpause: invalid schannel');schan.queue(['unpause']);return null;}
function glk_schannel_set_volume_ext(schan,vol,duration,notify){if(!schan||!gli_schannellist[schan.index])
throw('glk_schannel_set_volume_ext: invalid schannel');schan.queue(['set_volume',vol/0x10000,duration/1000]);return null;}
