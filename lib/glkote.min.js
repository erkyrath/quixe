GlkOte=function(){var game_interface=null;var windowport_id='windowport';var gameport_id='gameport';var generation=0;var disabled=false;var loading_visible=null;var windowdic=null;var current_metrics=null;var currently_focussed=false;var last_known_focus=0;var last_known_paging=0;var windows_paging_count=0;var resize_timer=null;var retry_timer=null;var perform_paging=true;var detect_external_links=false;var regex_external_links=null;var NBSP="\xa0";var max_buffer_length=200;var key_codes={KEY_BACKSPACE:8,KEY_TAB:9,KEY_RETURN:13,KEY_ESC:27,KEY_LEFT:37,KEY_UP:38,KEY_RIGHT:39,KEY_DOWN:40,KEY_DELETE:46,KEY_HOME:36,KEY_END:35,KEY_PAGEUP:33,KEY_PAGEDOWN:34,KEY_INSERT:45};var terminator_key_names={escape:key_codes.KEY_ESC,func1:112,func2:113,func3:114,func4:115,func5:116,func6:117,func7:118,func8:119,func9:120,func10:121,func11:122,func12:123};var terminator_key_values={};function glkote_init(iface){if(!iface&&window.Game)
...
game_interface=iface;if(!window.jQuery||!$.fn.jquery){glkote_error('The jQuery library has not been loaded.');return;}
var version=$.fn.jquery.split('.');if(version.length<2||version[0]<1||(version[0]==1&&version[1]<9)){glkote_error('This version of the jQuery library is too old. (Version '+$.fn.jquery+' found; 1.9.0 required.)');return;}
...
gameport_id=iface.gameport;var el=$('#'+windowport_id);if(!el.length){glkote_error('Cannot find windowport element #'+windowport_id+' in this document.');return;}
...
function measure_window(){var metrics={};var el,linesize,winsize,line1size,line2size,spansize;el=$('#'+gameport_id);if(!el.length)
return'Cannot find gameport element #'+gameport_id+' in this document.';metrics.width=el.width();metrics.height=el.height();el=$('#layouttest_grid');if(!el.length)
return'Cannot find layouttest_grid element for window measurement.';winsize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridspan');spansize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridline');line1size={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridline2');line2size={width:el.outerWidth(),height:el.outerHeight()};metrics.gridcharheight=($('#layouttest_gridline2').position().top
-$('#layouttest_gridline').position().top);metrics.gridcharwidth=($('#layouttest_gridspan').width()/8);metrics.gridmarginx=winsize.width-spansize.width;metrics.gridmarginy=winsize.height-(line1size.height+line2size.height);el=$('#layouttest_buffer');if(!el.length)
return'Cannot find layouttest_buffer element for window measurement.';winsize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferspan');spansize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferline');line1size={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferline2');line2size={width:el.outerWidth(),height:el.outerHeight()};metrics.buffercharheight=($('#layouttest_bufferline2').position().top
-$('#layouttest_bufferline').position().top);metrics.buffercharwidth=($('#layouttest_bufferspan').width()/8);metrics.buffermarginx=winsize.width-spansize.width;metrics.buffermarginy=winsize.height-(line1size.height+line2size.height);metrics.outspacingx=0;metrics.outspacingy=0;metrics.inspacingx=0;metrics.inspacingy=0;if(game_interface.spacing!=undefined){metrics.outspacingx=game_interface.spacing;metrics.outspacingy=game_interface.spacing;metrics.inspacingx=game_interface.spacing;metrics.inspacingy=game_interface.spacing;}
...
generation=arg.gen;if(disabled){$.each(windowdic,function(winid,win){if(win.inputel){win.inputel.prop('disabled',false);}});disabled=false;}
if(arg.input!=null)
...
accept_specialinput(arg.specialinput);$.each(windowdic,function(winid,win){if(win.type=='buffer'&&win.needscroll){win.needscroll=false;if(!win.needspaging){var frameel=win.frameel;if(!perform_paging){frameel.scrollTop(frameel.get(0).scrollHeight);win.needspaging=false;}
...
var moreel=$('#win'+win.id+'_moreprompt');if(!win.needspaging){if(moreel.length)
moreel.remove();}
else{if(!moreel.length){moreel=$('<div>',{id:'win'+win.id+'_moreprompt','class':'MorePrompt'});moreel.append('More');var morex=win.coords.right+20;var morey=win.coords.bottom;moreel.css({bottom:morey+'px',right:morex+'px'});$('#'+windowport_id).append(moreel);}}}}});readjust_paging_focus(false);disabled=false;if(arg.disable||arg.specialinput){disabled=true;$.each(windowdic,function(winid,win){if(win.inputel){win.inputel.prop('disabled',true);}});}
var newinputwin=0;if(!disabled&&!windows_paging_count){$.each(windowdic,function(winid,win){if(win.input){if(!newinputwin||win.id==last_known_focus)
newinputwin=win.id;}});}
...
function accept_windowset(arg){$.each(windowdic,function(winid,win){win.inplace=false;});$.map(arg,accept_one_window);var closewins=$.map(windowdic,function(win,winid){if(!win.inplace)
return win;});$.map(closewins,close_one_window);}
...
typeclass='BufferWindow';var rockclass='WindowRock_'+arg.rock;frameel=$('<div>',{id:'window'+arg.id,'class':'WindowFrame '+typeclass+' '+rockclass});frameel.data('winid',arg.id);frameel.on('mousedown',arg.id,evhan_window_mousedown);if(perform_paging&&win.type=='buffer')
frameel.on('scroll',arg.id,evhan_window_scroll);win.frameel=frameel;win.gridheight=0;win.gridwidth=0;win.input=null;win.inputel=null;win.terminators={};win.reqhyperlink=false;win.needscroll=false;win.needspaging=false;win.topunseen=0;win.coords={left:null,top:null,right:null,bottom:null};win.history=new Array();win.historypos=0;$('#'+windowport_id).append(frameel);}
...
if(arg.gridheight<win.gridheight){for(ix=arg.gridheight;ix<win.gridheight;ix++){var el=$('#win'+win.id+'_ln'+ix);if(el.length)
el.remove();}}
...
function close_one_window(win){win.frameel.remove();delete windowdic[win.id];win.frameel=null;var moreel=$('#win'+win.id+'_moreprompt');if(moreel.length)
moreel.remove();}
...
function accept_contentset(arg){$.map(arg,accept_one_content);}
...
win.needscroll=true;if(win.type=='grid'){var lines=arg.lines;var ix,sx;for(ix=0;ix<lines.length;ix++){var linearg=lines[ix];var linenum=linearg.line;var content=linearg.content;var lineel=$('#win'+win.id+'_ln'+linenum);if(!lineel.length){glkote_error('Got content for nonexistent line '+linenum+' of window '+arg.id+'.');continue;}
if(!content||!content.length){lineel.text(NBSP);}
else{lineel.empty();for(sx=0;sx<content.length;sx++){var rdesc=content[sx];var rstyle,rtext,rlink;if(rdesc.length===undefined){rstyle=rdesc.style;rtext=rdesc.text;rlink=rdesc.hyperlink;}
...
var cursel=$('#win'+win.id+'_cursor');if(cursel.length)
...
function accept_inputcancel(arg){var hasinput={};$.map(arg,function(argi){if(argi.type)
hasinput[argi.id]=argi;});$.each(windowdic,function(winid,win){if(win.input){var argi=hasinput[win.id];if(argi==null||argi.gen>win.input.gen){win.input=null;if(win.inputel){win.inputel.remove();win.inputel=null;}}}});}
function accept_inputset(arg){var hasinput={};var hashyperlink={};$.map(arg,function(argi){if(argi.type)
hasinput[argi.id]=argi;if(argi.hyperlink)
hashyperlink[argi.id]=true;});$.each(windowdic,function(tmpid,win){win.reqhyperlink=hashyperlink[win.id];var argi=hasinput[win.id];if(argi==null)
...
inputel.attr('autocapitalize','off');if(argi.type=='line'){inputel.on('keypress',evhan_input_keypress);inputel.on('keydown',evhan_input_keydown);if(argi.initial)
...
if(win.type=='grid'){var lineel=$('#win'+win.id+'_ln'+argi.ypos);if(!lineel.length){glkote_error('Window '+win.id+' has requested input at unknown line '+argi.ypos+'.');return;}
...
if(win.type=='buffer'){var cursel=$('#win'+win.id+'_cursor');if(!cursel.length){cursel=$('<span>',{id:'win'+win.id+'_cursor','class':'InvisibleCursor'});cursel.append(NBSP);win.frameel.append(cursel);}
...
function readjust_paging_focus(canfocus){windows_paging_count=0;var pageable_win=0;if(perform_paging){$.each(windowdic,function(tmpid,win){if(win.needspaging){windows_paging_count+=1;if(!pageable_win||win.id==last_known_paging)
pageable_win=win.id;}});}
if(windows_paging_count){last_known_paging=pageable_win;}
if(!windows_paging_count&&canfocus){var newinputwin=0;if(!disabled&&!windows_paging_count){$.each(windowdic,function(tmpid,win){if(win.input){if(!newinputwin||win.id==last_known_focus)
newinputwin=win.id;}});}
...
function inspect_deep(res){var keys=$.map(res,function(val,key){return key;});keys.sort();var els=$.map(keys,function(key){var val=res[key];if(jQuery.type(val)==='string')
val="'"+val+"'";else if(!(jQuery.type(val)==='number'))
val=inspect_deep(val);return key+':'+val;});return'{'+els.join(', ')+'}';}
function inspect_shallow(res){var keys=$.map(res,function(val,key){return key;});keys.sort();var els=$.map(keys,function(key){var val=res[key];if(jQuery.type(val)==='string')
...
if(!(type=='init'||type=='refresh'||type=='specialresponse')){$.each(windowdic,function(tmpid,win){var savepartial=(type!='line'&&type!='char')||(win.id!=winid);if(savepartial&&win.input&&win.input.type=='line'&&win.inputel&&win.inputel.val()){var partial=res.partial;if(!partial){partial={};res.partial=partial;};partial[win.id]=win.inputel.val();}});}
...
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(win.needspaging){if(frameel.scrollTop()+frameheight>=frameel.get(0).scrollHeight){win.needspaging=false;var moreel=$('#win'+win.id+'_moreprompt');if(moreel.length)
...
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(frameel.scrollTop()+frameheight>=frameel.get(0).scrollHeight){win.needspaging=false;var moreel=$('#win'+win.id+'_moreprompt');if(moreel.length)
