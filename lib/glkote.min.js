...
if(win.type=='buffer'){}
if(win.type=='graphics'){var el=$('#win'+win.id+'_canvas',dom_context);if(!el.length){win.graphwidth=arg.width;win.graphheight=arg.height;win.defcolor='#FFF';el=$('<canvas>',{id:'win'+win.id+'_canvas'});el.attr('width',win.graphwidth);el.attr('height',win.graphheight);win.frameel.append(el);}
else{if(win.graphwidth!=arg.width||win.graphheight!=arg.height){win.graphwidth=arg.width;win.graphheight=arg.height;el.attr('width',win.graphwidth);el.attr('height',win.graphheight);var canvas=el.get(0);if(canvas&&canvas.getContext){var ctx=canvas.getContext('2d');if(ctx){ctx.fillStyle=win.defcolor;ctx.fillRect(0,0,win.graphwidth,win.graphheight);ctx.fillStyle='#000000';}}
...
glkote_log('### perform_graphics_ops, '+graphics_draw_queue.length+' queued'+(loadedimg?' (plus image!)':'')+'.');while(graphics_draw_queue.length){var op=graphics_draw_queue[0];var win=windowdic[op.winid];if(!win){glkote_log('perform_graphics_ops: op for nonexistent window '+op.winid);graphics_draw_queue.shift();continue;}
...
ctx.fillStyle='#000000';break;case'image':if(!loadedimg){var imgurl=op.url;if(window.GiLoad&&GiLoad.get_image_url){var newurl=GiLoad.get_image_url(op.image);if(newurl)
imgurl=newurl;}
glkote_log('### setting up callback with url');var newimg=new Image();$(newimg).on('load',function(ev){perform_graphics_ops(newimg,ev);});$(newimg).on('error',function(ev){perform_graphics_ops(newimg,null);});newimg.src=imgurl;return;}
if(loadedev){ctx.drawImage(loadedimg,op.x,op.y,op.width,op.height);}
...
if(win.pair_key&&win.pair_key.type==Const.wintype_Graphics){split=win.pair_size;}
...
arr[0]=1;return 0;}
if(arr)
arr[0]=1;return 2;case 4:return 0;case 5:return 1;case 6:return 1;case 7:if(val==Const.wintype_TextBuffer)
...
break;case Const.wintype_Graphics:win.content.length=0;win.content.push({special:'fill'});break;}}
...
function gli_color_to_css(color){var res=(color&0xFFFFFF).toString(16);return'#'+res.toUpperCase();}
