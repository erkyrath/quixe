...
function file_notimplemented(){throw('streaming function not implemented in Dialog');}
...
$(window).on('storage',evhan_storage_changed);return{streaming:false,open:dialog_open,file_construct_ref:file_construct_ref,file_ref_exists:file_ref_exists,file_remove_ref:file_remove_ref,file_write:file_write,file_read:file_read,file_fopen:file_notimplemented,file_fclose:file_notimplemented,file_fread:file_notimplemented,file_fwrite:file_notimplemented};}();Glk=function(){var VM=null;var has_canvas;var has_exited=false;var ui_disabled=false;var ui_specialinput=null;var ui_specialcallback=null;var event_generation=0;var current_partial_inputs=null;var current_partial_outputs=null;function init(vm_options){has_canvas=(document.createElement('canvas').getContext!=undefined);VM=vm_options.vm;if(window.GiDispa)
...
else if(str.type==strtype_File){if(str.fstream){Dialog.file_fclose(str.fstream);str.fstream=null;}}
if(window.GiDispa)
...
ch=63;}
str.writecount+=1;switch(str.type){case strtype_File:if(str.streaming){if(!str.unicode){Dialog.file_fwrite(str.fstream,String.fromCharCode(ch));}
else{if(!str.isbinary){Dialog.file_fwrite(str.fstream,String.fromCharCode(ch));}
else{Dialog.file_fwrite(str.fstream,String.fromCharCode(0));Dialog.file_fwrite(str.fstream,String.fromCharCode(0));Dialog.file_fwrite(str.fstream,String.fromCharCode(0));Dialog.file_fwrite(str.fstream,String.fromCharCode(ch));}}
break;}
...
str.writecount+=arr.length;switch(str.type){case strtype_File:if(str.streaming){if(!str.unicode){if(allbytes)
val=String.fromCharCode.apply(this,arr);else
val=UniArrayToString(arr);Dialog.file_fwrite(str.fstream,val);}
else{if(!str.isbinary){if(allbytes)
val=String.fromCharCode.apply(this,arr);else
val=UniArrayToString(arr);Dialog.file_fwrite(str.fstream,val);}
else{for(ix=0;ix<arr.length;ix++){val=arr[ix];Dialog.file_fwrite(str.fstream,String.fromCharCode(((val>>24)&0xFF)));Dialog.file_fwrite(str.fstream,String.fromCharCode(((val>>16)&0xFF)));Dialog.file_fwrite(str.fstream,String.fromCharCode(((val>>8)&0xFF)));Dialog.file_fwrite(str.fstream,String.fromCharCode(((val)&0xFF)));}}}
break;}
...
return 63;return ch;}
case strtype_File:if(str.streaming){if(!str.unicode){var buf=Dialog.file_fread(str.fstream,1);if(!buf||!buf.length)
return-1;str.readcount++;return buf[0];}
else{if(!str.isbinary){var val0,val1,val2,val3;var buf=Dialog.file_fread(str.fstream,1);if(!buf||!buf.length)
return-1;val0=buf[0];if(val0<0x80){ch=val0;}
else{buf=Dialog.file_fread(str.fstream,1);if(!buf||!buf.length)
return-1;val1=buf[0];if((val1&0xC0)!=0x80)
return-1;if((val0&0xE0)==0xC0){ch=(val0&0x1F)<<6;ch|=(val1&0x3F);}
else{buf=Dialog.file_fread(str.fstream,1);if(!buf||!buf.length)
return-1;val2=buf[0];if((val2&0xC0)!=0x80)
return-1;if((val0&0xF0)==0xE0){ch=(((val0&0xF)<<12)&0x0000F000);ch|=(((val1&0x3F)<<6)&0x00000FC0);ch|=(((val2&0x3F))&0x0000003F);}
else if((val0&0xF0)==0xF0){buf=Dialog.file_fread(str.fstream,1);if(!buf||!buf.length)
return-1;val3=buf[0];if((val3&0xC0)!=0x80)
return-1;ch=(((val0&0x7)<<18)&0x1C0000);ch|=(((val1&0x3F)<<12)&0x03F000);ch|=(((val2&0x3F)<<6)&0x000FC0);ch|=(((val3&0x3F))&0x00003F);}
else{return-1;}}}}
else{var buf=Dialog.file_fread(str.fstream,4);if(!buf||buf.length<4)
return-1;ch=(buf[0]<<24);ch|=(buf[1]<<16);ch|=(buf[2]<<8);ch|=buf[3];}
str.readcount++;ch>>>=0;if(!want_unicode&&ch>=0x100)
return 63;return ch;}}
...
throw('glk_put_jstring: invalid stream');str.writecount+=val.length;switch(str.type){case strtype_File:if(str.streaming){Dialog.file_fwrite(str.fstream,val);break;}
...
else{var fstream=Dialog.file_fopen(fmode,fref.ref);}
...
str.bufpos=0;}
else{str.streaming=true;str.fstream=fstream;}
return str;}
