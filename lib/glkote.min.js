...
function measure_window(){var metrics={};var el,linesize,winsize,line1size,line2size,spansize;el=$('#'+gameport_id,dom_context);if(!el.length)
return'Cannot find gameport element #'+gameport_id+' in this document.';metrics.width=el.width();metrics.height=el.height();el=$('#layouttest_grid',dom_context);if(!el.length)
return'Cannot find layouttest_grid element for window measurement.';winsize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridspan',dom_context);spansize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridline',dom_context);line1size={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_gridline2',dom_context);line2size={width:el.outerWidth(),height:el.outerHeight()};metrics.gridcharheight=($('#layouttest_gridline2',dom_context).position().top
-$('#layouttest_gridline',dom_context).position().top);metrics.gridcharwidth=($('#layouttest_gridspan',dom_context).width()/8);metrics.gridmarginx=winsize.width-spansize.width;metrics.gridmarginy=winsize.height-(line1size.height+line2size.height);el=$('#layouttest_buffer',dom_context);if(!el.length)
return'Cannot find layouttest_buffer element for window measurement.';winsize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferspan',dom_context);spansize={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferline',dom_context);line1size={width:el.outerWidth(),height:el.outerHeight()};el=$('#layouttest_bufferline2',dom_context);line2size={width:el.outerWidth(),height:el.outerHeight()};metrics.buffercharheight=($('#layouttest_bufferline2',dom_context).position().top
-$('#layouttest_bufferline',dom_context).position().top);metrics.buffercharwidth=($('#layouttest_bufferspan',dom_context).width()/8);metrics.buffermarginx=winsize.width-spansize.width;metrics.buffermarginy=winsize.height-(line1size.height+line2size.height);metrics.outspacingx=0;metrics.outspacingy=0;metrics.inspacingx=0;metrics.inspacingy=0;if(game_interface.spacing!=undefined){metrics.outspacingx=game_interface.spacing;metrics.outspacingy=game_interface.spacing;metrics.inspacingx=game_interface.spacing;metrics.inspacingy=game_interface.spacing;}
...
for(sx=0;sx<content.length;sx++){var rdesc=content[sx];var rstyle,rtext,rlink;if(jQuery.type(rdesc)==='object'){if(rdesc.special!==undefined){if(rdesc.special=='image'){var el=$('<img>',{src:rdesc.url,width:''+rdesc.width,height:''+rdesc.height});if(rdesc.alttext)
...
return null;var info=GiLoad.get_image_info(imgid);if(info!==null){if(widthref)
...
return 0;var info=GiLoad.get_image_info(imgid);if(info===null)
...
img.alignment=alignment;gli_window_buffer_put_special(win,img);return 1;}
return 0;}
...
throw('glk_window_flow_break: invalid window');gli_window_buffer_put_special(win,undefined,true);}
