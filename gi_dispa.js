...
/* Some literal ArgInt and ArgChar objects, used when registering
   retained arrays. */
var arg_int_unsigned = new ArgInt(false);
var arg_int_signed = new ArgInt(true);
var arg_char_unsigned = new ArgChar(false); /* also used for "native" */
...
function build_function(func) {
    ...
    for (ix=0; ix<form.args.length; ix++) {
        ...
        if ((arg instanceof ArgInt)
            ...
            if (arg.retained)
                out.push('  retain_array('+tmpvar+', callargs['+argpos+'], glklen, '+refarg.literal+');');
            out.push('}');
            argpos += 2;
        }
...
/* Retain one array. The address and length are where it will go in VM
   memory. The arg is an ArgInt or ArgChar object, describing the array
   entries.
*/
function retain_array(arr, addr, len, arg) {
    var ix;
    var obj = { arr:arr, addr:addr, len:len, arg:arg };
